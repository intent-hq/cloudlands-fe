...
let cdpHelpersCode: string = '';

// Console log buffer (circular buffer with max 1000 entries)
...
function loadCdpHelpers(): void {
  try {
    ...
    const helpersPath = path.join(path.dirname(__filename), 'cdp-helpers.js');
    logToStderr('DEBUG', `Loading CDP helpers from: ${helpersPath}`);
    logToStderr('DEBUG', `__filename: ${__filename}`);
    cdpHelpersCode = fs.readFileSync(helpersPath, 'utf-8');
...
async function connectCDP(): Promise<void> {
  ...
  try {
    ...
    if (appTarget) {
      ...
      cdpClient = await CDP({ port, target: appTarget.id });
    } else {
...
async function setupConsoleCapture(): Promise<void> {
  if (!cdpClient) {
    return;
  }
...
async function ensureCDPConnected(): Promise<void> {
  if (cdpClient) {
  ...
  throw new Error(
    `Failed to reconnect to CDP after ${retryDelays.length} attempts.\n` +
    ...
    inputSchema: {
      ...
      properties: {
        script: {
          type: 'string',
          description:
            'JavaScript code with access to the cdp Playwright-style API. Runs in async function context. Use return to send results back.',
        },
      },
      required: ['script'],
    },
    ...
    inputSchema: {
      ...
      properties: {
        selector: {
          type: 'string',
          description:
            'Optional CSS selector to get accessibility tree for a specific element and its descendants. If not provided, returns the full page accessibility tree. Using a selector is more efficient for large pages.',
        },
        depth: {
          type: 'integer',
          description:
            'Maximum depth of descendants to retrieve when getting the full tree (no selector). If omitted, returns the complete tree. Only applies when selector is not provided.',
        },
        fetchRelatives: {
          type: 'boolean',
          description:
            'When using a selector, whether to include parent and sibling nodes for context. Default: false.',
        },
      },
      required: [],
    },
    ...
    inputSchema: {
      ...
      properties: {
        selector: {
          type: 'string',
          description:
            'Optional CSS selector to screenshot a specific element. If not provided, captures the full viewport.',
        },
        fullPage: {
          type: 'boolean',
          description:
            'If true, captures the full scrollable page instead of just the viewport. Default: false. Ignored when selector is provided.',
        },
        quality: {
          type: 'integer',
          description:
            'Image quality for JPEG format (0-100). Default: 80. Only applies when format is jpeg.',
        },
        format: {
          type: 'string',
    ...
    inputSchema: {
      ...
      properties: {
        ignoreCache: {
          type: 'boolean',
          description:
            'If true, bypasses the browser cache and forces a full reload. Default: false.',
        },
      },
      required: [],
    },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        selector: {
          type: 'string',
          description:
            'Optional CSS selector to wait for. If provided, waits until the element exists in the DOM.',
        },
        timeout: {
          type: 'integer',
          description:
            'Maximum time to wait for selector in milliseconds. Default: 5000. Only applies when selector is provided.',
        },
      },
      required: [],
    },
...
// Tool handlers
async function handleCdpHello(): Promise<any> {
...
function handleCdpApiReference(): any {
  return {
    content: [
      {
        type: 'text',
...
async function handleCdpRunScript(args: any): Promise<any> {
  await ensureCDPConnected();
...
async function handleCdpGetDom(args: any): Promise<any> {
  await ensureCDPConnected();
...
async function handleCdpGetConsoleLogs(args: any): Promise<any> {
  ...
  const summary =
    `Retrieved ${recentLogs.length} console log(s)` +
...
async function handleCdpScreenshot(args: any): Promise<any> {
  await ensureCDPConnected();
...
async function handleCdpReload(args: any): Promise<any> {
  await ensureCDPConnected();
...
async function handleCdpWait(args: any): Promise<any> {
  await ensureCDPConnected();
...
async function handleCdpGetAccessibilityTree(args: any): Promise<any> {
  await ensureCDPConnected();
