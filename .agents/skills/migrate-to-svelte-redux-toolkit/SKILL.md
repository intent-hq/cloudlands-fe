---
name: migrate-to-svelte-redux-toolkit
description: >-
  ...
  svelte-redux-toolkit. Root index that routes to assessment, setup, per-primitive
...
Migrating from Svelte stores to `svelte-redux-toolkit` is driven by two rules:
...
export const cartCleanupEvidence: CleanupEvidence = {
  ...
  remainingImports: [],
  passThroughWrappers: [],
  testsRun: ["npm run validate:architecture", "npm run validate:skill-examples"],
};
```
