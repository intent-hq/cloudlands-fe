---
name: svelte-redux-toolkit/selectors
description: >-
  store.createSelector((state, ...args) => value) is the public app-local
  selector factory tied to a configured Store. Generic/shared selector helpers
  should accept a configured Store rather than importing a standalone factory.
...
> Operational guidance for selector work. Full API reference and examples: `docs/SELECTORS.md`. Source: `src/utils/selectors/create-selector.ts` and `skills/svelte-redux-toolkit/SKILL.md` §3.
...
- `store.createSelector(...)` for app-local selectors next to a configured `Store`; it preserves `StoreState<typeof store>` inference after reducer registration and is the default production app-code pattern.
- Generic/shared selector helpers should accept a configured `Store` and create selectors through `store.createSelector(...)`; do not import a standalone selector factory from the package.
...
- Do not import standalone selector factories from package subpaths; selector creation is Store-bound public API.
...
### 5. Pass a configured Store into shared selector factories
...
- `svelte-redux-toolkit/selector-lifecycle/SKILL.md` — lifecycle crash prevention.
- `svelte-redux-toolkit/selector-channels/SKILL.md` — reacting to selector changes from sagas.
- `svelte-redux-toolkit/collections/SKILL.md` — normalized state shape and collection utilities used by selectors.
- `svelte-redux-toolkit/state-integrity/SKILL.md` — canonical derived-value ownership.
- `svelte-redux-toolkit/testing/SKILL.md` — `.select(state)` testing rule.