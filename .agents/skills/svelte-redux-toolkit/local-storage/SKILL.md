---
name: svelte-redux-toolkit/local-storage
description: >-
...
Source: `skills/svelte-redux-toolkit/SKILL.md §15`, `examples/utils/safe-local-storage-saga.ts`.
...
- ❌ Never import localStorage helpers from `svelte-redux-toolkit/saga` or package utility subpaths.
...
Source: `skills/svelte-redux-toolkit/SKILL.md §15`. Priority: **HIGH**.
...
Source: `skills/svelte-redux-toolkit/SKILL.md §15`. Priority: **MEDIUM**.

## See also

- `svelte-redux-toolkit/sagas` — `takeEvery` patterns and the saga lifecycle this code runs in.
- `svelte-redux-toolkit/core-policy` — rule that side effects live in sagas, not components.
- `svelte-redux-toolkit/state-serialization` — keep state JSON-serializable so `JSON.stringify` round-trips cleanly.