---
name: svelte-redux-toolkit/debugging
description: >-
...
sources:
  ...
  - augmentcode/svelte-redux-toolkit:src/global-dev-tools.ts
triggers:
  - svelteRedux api
...
`svelte-redux-toolkit/state-serialization` for the allowed and forbidden
state shapes.
...
*Source: `src/store.ts` and `src/global-dev-tools.ts`.*
...
*Source: `src/store.ts` (`Store.state`, `Store.dispatch`).*

## 6. See also

- `svelte-redux-toolkit/component-integration` — `store.init()`, why
  multiple layouts calling `store.init()` is the usual
  "multiple stores initialized" root cause.
- `svelte-redux-toolkit/reducers` — the same-reference on no-op contract
  that reducer tests should enforce.
- `svelte-redux-toolkit/state-serialization` — the serializability rule
  reducers and tests should preserve.
- `svelte-redux-toolkit/testing` — assertions that mirror reducer and
