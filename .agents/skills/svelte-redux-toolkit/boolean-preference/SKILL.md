---
name: svelte-redux-toolkit/boolean-preference
description: >-
  ...
  boolean. Use it instead of hand-writing setX / toggleX pairs. Source:
...
sources:
  - augmentcode/svelte-redux-toolkit:src/utils/store/boolean-preference.ts
...
From `src/utils/store/boolean-preference.ts`:
...
Pair with an app-local helper following `svelte-redux-toolkit/local-storage` to persist the flag:
...
*Source: `skills/svelte-redux-toolkit/SKILL.md §11`.*
...
*Source: `src/utils/store/boolean-preference.ts` (`BooleanFieldKey<S>` constraint).*

## 5. See also

- `svelte-redux-toolkit/reducers` — `createReducer` builder and the
  `same-reference on no-op` rule.
- `svelte-redux-toolkit/actions` — `createAction<[Params]>` tuple payloads.
- `svelte-redux-toolkit/local-storage` — persistence saga for preference fields.
