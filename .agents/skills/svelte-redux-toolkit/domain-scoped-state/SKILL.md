---
name: svelte-redux-toolkit/domain-scoped-state
description: >-
  ...
  drop domains (returning the same reference when the id was absent). Source:
  src/utils/store/domain-scoped.ts, skills/svelte-redux-toolkit/SKILL.md §10.
type: sub-skill
...
sources:
  - augmentcode/svelte-redux-toolkit:src/utils/store/domain-scoped.ts
...
From `src/utils/store/domain-scoped.ts`:
...
import { createDomainScopedHelpers } from "svelte-redux-toolkit/utils/store/domain-scoped";
...
*Source: `skills/svelte-redux-toolkit/SKILL.md §10`.*
...
*Source: `skills/svelte-redux-toolkit/SKILL.md §10, §12`. See also
...
*Source: `src/utils/store/domain-scoped.ts` (`DomainScopedState<T>` shape).*
...
- `svelte-redux-toolkit/reducers` — pure-update rules, `createReducer` builder.
- `svelte-redux-toolkit/state-serialization` — why `emptyState` must be
  structured-cloneable.
- `svelte-redux-toolkit/collections` — the usual shape of per-domain data.
