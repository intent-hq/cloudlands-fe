---
name: svelte-redux-toolkit/core-policy
description: >-
  Top-level architectural rules for svelte-redux-toolkit. Redux owns all shared
...
> These rules are the load-bearing contract for every slice, saga, and component in the package. Quoted verbatim from skills/svelte-redux-toolkit/SKILL.md §1 and docs/ARCHITECTURE.md → Core Principles.
...
3. **Side effects in sagas, not components.** API calls, localStorage, timers, event listeners → sagas. Use `$effect` only for DOM-local work (focus, scroll, measurements).
4. **State must be serializable.** No `Date`, `Map`, `Set`, `RegExp`, `Promise`, `Function`, class instances, `Symbol`. Use plain objects, arrays, strings, numbers, booleans, `null`, `undefined`. See `svelte-redux-toolkit/state-serialization/SKILL.md`.
5. **State is canonical only.** Never store derived fields, duplicated entity copies, parallel arrays/maps for the same records, or values a selector can compute. See `svelte-redux-toolkit/state-integrity/SKILL.md`.
6. **Arrays hold primitives only.** For entity/object storage, always use `Collection<T, K>`. Never store `Item[]` in state — use Collections for O(1) lookups and normalized data. See `svelte-redux-toolkit/collections/SKILL.md`.
7. **Actions/selectors/sagas have one owner.** Before adding any action, selector, watcher, or saga registration, search for an existing owner and import/compose/extend it instead of duplicating it. See `svelte-redux-toolkit/state-integrity/SKILL.md`.
8. **Svelte store migration on contact.** If you encounter `.store.svelte.ts`, do not expand it — migrate to Redux. See `migrate-to-svelte-redux-toolkit/SKILL.md`.
...
Before adding Redux state, actions, selectors, or sagas, load `svelte-redux-toolkit/state-integrity` and search for existing canonical owners. The handoff must list the searched paths/terms and say whether the change reused, extended, or created the canonical owner.
...
Source: `svelte-redux-toolkit/state-integrity/SKILL.md` · **Priority: CRITICAL**
...
Source: `skills/svelte-redux-toolkit/SKILL.md` §1, README.md · **Priority: CRITICAL**
...
Source: `skills/svelte-redux-toolkit/SKILL.md` §1, §8 · **Priority: HIGH**
...
- `svelte-redux-toolkit/import-boundaries/SKILL.md` — structural enforcement of the policy
- `svelte-redux-toolkit/state-integrity/SKILL.md` — canonical state and owner deduplication rules
- `svelte-redux-toolkit/file-structure/SKILL.md` — where each file lives
- `svelte-redux-toolkit/state-serialization/SKILL.md` — allowed/forbidden state types
- `migrate-to-svelte-redux-toolkit/SKILL.md` — migrating existing Svelte stores