...
name: setup
description: >-
  Canonical first-time ag-redux-toolkit setup entry. Start here for greenfield app
...
---
# Init ag-redux-toolkit — Canonical Greenfield Setup
...
> Canonical setup guide for bootstrapping a new app with the ag-redux-toolkit package. Follow the branch that matches the one concrete Store family chosen for the app.
...
| --- | --- | --- | --- |
| Concrete Svelte/SvelteKit evidence: svelte.config.*, .svelte files, SvelteKit +layout/+page, imports from svelte, $selector template reads, Svelte readable expectations | Svelte Store family | Store from @augmentcode/ag-redux-toolkit/svelte-store | ../svelte/SKILL.md, ../svelte/component-integration/SKILL.md, ../svelte/selectors/SKILL.md |
| Concrete React evidence: React dependencies in the app path, JSX/TSX React components/hooks, imports from react, ReactStore, Preact React signal selectors, direct signal reads, or necessary .useValue(...args) fallback reads | React Store family | ReactStore from @augmentcode/ag-redux-toolkit/react-store | ../react/SKILL.md, ../react/store/SKILL.md, ../react/selectors/SKILL.md |
| Node services, server routes, background workers, CLIs, scripts, test harnesses, no-UI paths, Kefir/observable selector arguments, or no concrete Svelte/React evidence | Streaming Store family by default | StreamingStore from @augmentcode/ag-redux-toolkit/streaming-store | ../streaming/SKILL.md, ../streaming/store/SKILL.md, ../streaming/selectors/SKILL.md, ../streaming/selector-lifecycle/SKILL.md |
...
| --- | --- | --- | --- |
| Store import and instance | import { Store } from '@augmentcode/ag-redux-toolkit/svelte-store'; export const store = new Store({ counter: counterReducer }) | import { ReactStore } from '@augmentcode/ag-redux-toolkit/react-store'; export const reactStore = new ReactStore({ counter: counterReducer }) | import { StreamingStore } from '@augmentcode/ag-redux-toolkit/streaming-store'; export const streamStore = new StreamingStore({ counter: counterReducer }) |
...
```ts
import { ReactStore } from '@augmentcode/ag-redux-toolkit/react-store';
import type { StoreState } from '@augmentcode/ag-redux-toolkit/types';
...
```bash
npm install @augmentcode/ag-redux-toolkit redux redux-saga typed-redux-saga fast-equals
```
...
The equivalent npm exec form is `npm exec -- ag-redux-toolkit <command>`. Use this repository's maintainer `npm run validate:*` scripts only when working inside the package repository. npm 7+ does not run dependency uninstall lifecycle scripts, so run the package cleanup command before uninstalling when installed `.agents/skills/ag-redux-toolkit/` files should be removed. Cleanup is manifest-driven: it removes only the files listed in `installed-skills.yml` plus the manifest itself, prints a clear no-op message when no manifest exists, and removes legacy flat `.agents/skills/` package copies as a one-time migration:
...
```bash
npm uninstall @augmentcode/ag-redux-toolkit
...
| --- | --- | --- |
| Create action (no payload) | @augmentcode/ag-redux-toolkit/utils/store/create-action | createAction('slice/name') |
| Create action (with payload) | @augmentcode/ag-redux-toolkit/utils/store/create-action | createAction<[string, number]>('slice/name') |
| Create reducer | @augmentcode/ag-redux-toolkit/utils/store/create-reducer | createReducer<State>(init).with(action, handler) |
...
The user creates their own slice files under `src/lib/store/slices/<name>/` — these are your application code, not package source. Import builder APIs such as `createAction` and `createReducer` from their explicit utility leaf subpaths (`@augmentcode/ag-redux-toolkit/utils/store/create-action` and `@augmentcode/ag-redux-toolkit/utils/store/create-reducer`); import the configured app Store instance (`store`, `reactStore`, or `streamStore`) and use that instance for `createSelector(...)` and `dispatch(...)`.
