...
description: >-
  ...
  workers, CLIs, test harnesses, apps or code paths without concrete Svelte
  evidence, and cases where selector direct calls should return Kefir streams.
  Route shared Redux/redux-saga work to core and Svelte frontend/readable work to
  svelte only when Svelte/SvelteKit evidence exists. Never mix this concrete
  Streaming Store family with Store/readable/component patterns in one app.
type: core
requires:
  - core
sources:
...
Use this root for Streaming-specific `ag-redux-toolkit` work and as thedefault package route when the touched code path has no concrete Svelte/SvelteKitevidence. It covers Node/server environments, background workers, CLIs, testharnesses, apps without Svelte integration, and the Kefir/observable Storevariant. Generic Redux/redux-saga guidance remains in `../core/`, andSvelte-readable selector/component guidance remains in `../svelte/` only forfrontend-facing Svelte paths.

> Store split truth: StreamingStore is the Kefir/observable variant exportedfrom ag-redux-toolkit/streaming-store. Store is the Svelte-readable class exported from ag-redux-toolkit/svelte-store.
...
- Do **not** apply `Store`, Svelte `Readable`, `$selector` template,Svelte component integration, Svelte setup, or Svelte selector lifecyclepatterns inside that same app/package/code path.
- In a mixed repository, a separate frontend Svelte/SvelteKit app may use`../svelte/`, but keep that choice isolated from this Streaming app.
...
- Do not use Svelte readable, Svelte component, or Svelte lifecyclesetup patterns as implementation guidance for this app.
...
- Use `StreamingStore` only from `ag-redux-toolkit/streaming-store`.
...
- Direct selector calls return Kefir observables. They are not Svelte readables,do not use `$selector` template syntax, and do not depend on Svelte context.
