...
Use this skill for generic redux-saga API behavior. When editing this repository's`ag-redux-toolkit` code, also follow the package-specific core saga skills in sibling `../*` skill folders, especially the `typed-redux-saga` `yield*`conventions and canonical watcher ownership rules.
