...
describe('sanitizeCommandForDisplay', () => {
  ...
  it('redacts password in mongodb connection string', () => {
    expect(sanitizeCommandForDisplay('mongosh mongodb://root:secret@localhost:27017/admin')).toBe(
      'mongosh mongodb://root:***@localhost:27017/admin',
    );
  });

  it('redacts password in mongodb+srv connection string', () => {
    expect(sanitizeCommandForDisplay('mongosh mongodb+srv://user:pass@cluster.mongodb.net')).toBe(
      'mongosh mongodb+srv://user:***@cluster.mongodb.net',
    );
  });

  it('redacts password in git+ssh URL', () => {
    expect(sanitizeCommandForDisplay('git clone git+ssh://user:pass@github.com/repo.git')).toBe(
      'git clone git+ssh://user:***@github.com/repo.git',
    );
  });

  it('redacts password in URL with email query param (greedy @ fix)', () => {
    expect(sanitizeCommandForDisplay('curl https://user:pass@host.com/path?email=a@b.com')).toBe(
      'curl https://user:***@host.com/path?email=a@b.com',
    );
  });

  // --- curl -u / --user ---
  it('redacts curl -u user:pass', () => {
    expect(sanitizeCommandForDisplay('curl -u admin:secret123 https://api.example.com')).toBe(
      'curl -u admin:*** https://api.example.com',
    );
  });

  it('redacts curl --user user:pass', () => {
    expect(sanitizeCommandForDisplay('curl --user admin:secret123 https://api.example.com')).toBe(
      'curl --user admin:*** https://api.example.com',
    );
  });
