...
describe('sanitizeCommandForDisplay - Adversarial Tests', () => {
  ...
  describe('Whitespace and formatting bypasses', () => {
    it('BYPASS: tab between flag and value', () => {
      expect(sanitizeCommandForDisplay('curl --token\tghp_secret123')).toBe('curl --token ***');
    });

    it('BYPASS: newline between flag and value', () => {
      expect(sanitizeCommandForDisplay('curl --token\nghp_secret123')).toBe('curl --token ***');
    });
    ...
    it('BYPASS: mixed case flag names', () => {
      expect(sanitizeCommandForDisplay('curl --Password=hunter2')).toBe('curl --Password=***');
    });
  ...
  describe('Quote and escape bypasses', () => {
    ...
    it('BYPASS: backticks in value', () => {
      expect(sanitizeCommandForDisplay('export API_KEY=`echo secret`')).toBe('export API_KEY=***');
    });
  });
  ...
  describe('Special characters in values', () => {
    ...
    it('BYPASS: unicode characters in password', () => {
      expect(sanitizeCommandForDisplay('export API_KEY=café123🔑')).toBe('export API_KEY=***');
    });

    it('BYPASS: password with equals sign', () => {
      expect(sanitizeCommandForDisplay('export PASSWORD="pass=word"')).toBe('export PASSWORD=***');
    });
  });
