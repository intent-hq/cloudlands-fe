...
const AUTH_HEADER_RE = /(-[Hh]\s+|--header\s+)(['"])(Authorization:\s*)[^'"]*(\2)/gi;
...
const CURL_USER_RE = /(curl\s+(?:\S+\s+)*(?:-u|--user)[=\s]\s*)(["']?)([^\s"':]+):([^\s"']+)\2/gi;
...
const SHORT_FLAG_RE = /((?:mysql|mongosh?|redis-cli|sshpass)\s+(?:\S+\s+)*-[pP])\s+(\S+)/gi;
