...
export interface AgentSession {
   ...
   *  `agent.list` rows only on the retired-row read (`retiredOnly: true`,
   *  v8.2); omitted on active rows, never `null`. A retired session is inert
   *  daemon-side (sends, queueing, watches, retry all reject) until
   *  `agent.restore` clears it. */
  retiredAt?: string;
