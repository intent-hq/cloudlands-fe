...
export class AgentTestHarness extends EventEmitter {
   ...
   */
  async testPersistence(agentId: AgentId): Promise<{
    success: boolean;
    saveTime: number;
    loadTime: number;
    ...
    const operation = this.startOperation('testPersistence');
    const errors: string[] = [];
    let saveTime = 0;
    let loadTime = 0;
    ...
    try {
      ...
      if (!session) {
        session = await this.createAgent({ name: `persistence-test-${agentId}` });
      }

      // Test save operation
      const saveStart = Date.now();
      const saveResult = await unifiedPersistence.saveAgent(session);
      ...
      const loadResult = await unifiedPersistence.loadAgent(session.id, session.workspaceId);
      ...
      if (!loadResult.success) {
        ...
        integrityCheck =
          loadResult.data.id === session.id &&
          loadResult.data.name === session.name &&
          loadResult.data.messages.length === session.messages.length;
      ...
      // Test health check
      const healthResult = await unifiedPersistence.performHealthCheck();
      if (!healthResult.healthy) {
        errors.push(`Health check failed: ${healthResult.issues.map((i) => i.message).join(', ')}`);
      }

      this.endOperation(operation, errors.length === 0);
      ...
      if (errors.length > 0) {
        console.error('[testPersistence] Errors:', errors);
      }
   ...
   */
  async testCorruptionRecovery(): Promise<{
    success: boolean;
    ...
    const operation = this.startOperation('testCorruptionRecovery');

    try {
      // Import persistence and file system modules
      const { unifiedPersistence } = await import('../main/agent-persistence');
      ...
      // Create test agents and save them to the temp directory
      const testAgents: AgentSession[] = [];
      for (let i = 0; i < 5; i++) {
        const agent = await this.createAgent({ name: `corruption-test-${i}` });
        // Save to the temp directory
        const agentPath = path.join(agentsPath, `${agent.id}.json`);
        const agentData = JSON.stringify(agent, null, 2);
        await fs.writeFile(agentPath, agentData, 'utf-8');
        ...
        await fs.writeFile(backupPath, agentData, 'utf-8');
      ...
      for (let i = 0; i < 2 && i < testAgents.length; i++) {
        const agentPath = path.join(agentsPath, `${testAgents[i].id}.json`);
        try {
          // Write invalid JSON to corrupt the file
          await fs.writeFile(agentPath, '{ invalid json content', 'utf-8');
          corruptedIds.push(testAgents[i].id);
        } catch (error) {
          this.recordError('execution', error as Error, { phase: 'corruption' });
        }
      }

      // Check for corruption
      const corruptionCheck = await unifiedPersistence.checkForCorruption(workspacePath);

      // Attempt recovery
      const recoveryResult = await unifiedPersistence.recoverFromCorruption(workspacePath);
      ...
      this.endOperation(operation, recoveryResult.success);

      return {
        success: recoveryResult.success,
        corruptedFiles: corruptionCheck.corruptedFiles.length,
        recoveredFiles: recoveryResult.filesRecovered,
        lostFiles: recoveryResult.filesLost,
