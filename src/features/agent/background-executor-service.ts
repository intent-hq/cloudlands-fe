/**
 * Background executor saga — the root-owned consumer for the
 ...
 * code review, walkthrough) without changing any dispatch site.
 *
 ...
 * saga's semantics).
 *
 * The root saga owns the run generations so cancellation and restart reset
 * all transient execution bookkeeping.
 */
...
/** Lazily pull the toast lib so startup does not eagerly load it. */
...
export function* executeBackgroundAgentWorker(
  runGenerations: Map<string, number>,
  ...
  const generation = bumpGeneration(runGenerations, workspaceId, executorType);
  ...
  try {
    ...
    if (!isCurrentGeneration(runGenerations, workspaceId, executorType, generation)) return;
    ...
    const response = yield* call(backendRequest<CompleteOnceResult>, 'agent.completeOnce', params, {
    ...
    if (isUnavailable(response)) {
      ...
      yield* call(showErrorToast, m.bgExecutor_service_generateFailed_error(), reason);
      return;
    }
    ...
    yield* call(showErrorToast, m.bgExecutor_service_generateFailed_error(), message);
  }
}
...
export function* cancelBackgroundAgentWorker(
  runGenerations: Map<string, number>,
  action: ReturnType<typeof cancelExecution>,
) {
  const [workspaceId, executorType] = action.payload;
  bumpGeneration(runGenerations, workspaceId, executorType);
  yield* put(setExecutorState(workspaceId, executorType, { status: 'cancelled' }));
}

/**
 * Root-owned saga wiring `bgExecutor/execute` and `bgExecutor/cancel` to the
 * §5.32 one-shot completion pipeline.
 */
export function* backgroundAgentExecutorSaga() {
  const runGenerations = new Map<string, number>();
  yield* all([
    takeEvery(executeBackgroundAgent, executeBackgroundAgentWorker, runGenerations),
    takeEvery(cancelExecution, cancelBackgroundAgentWorker, runGenerations),
  ]);
}
