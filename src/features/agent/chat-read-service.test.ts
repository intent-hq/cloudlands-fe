...
// against the REAL configured store so dedup and upsert hydration are exercised
// end to end. READ-ONLY: only `get`/`getConversation`/
// `subscribeSnapshot`.
vi.mock('$lib/client', () => ({
  appClient: {
    agents: {
...
const conversation = (messages: AgentMessage[], nextToken: string | null = null) => ({
  messages,
...
describe('chatReadService (fake seam, real store)', () => {
  ...
  it('loadChatTranscript fetches session + transcript and hydrates messages', async () => {
    ...
    agentsApi.getConversation.mockResolvedValueOnce(
      conversation([makeMessage('m1', 'hi')]) as never,
    );
  ...
  it('leaves any prior transcript intact when the conversation read fails', async () => {
    ...
    agentsApi.getConversation.mockResolvedValueOnce(
      conversation([makeMessage('prior', 'p')]) as never,
    );
    await loadChatTranscript(agentId);
  ...
  it('coalesces concurrent loads into one shared fetch plus a single rerun', async () => {
    ...
    agentsApi.getConversation.mockResolvedValue(
      conversation([makeMessage('c', 'shared')]) as never,
    );
  ...
  it('merges chat.subscribe snapshot in-flight message into hydrated transcript', async () => {
    ...
    const inFlightAssistant: AgentMessage = {
      ...
      isStreaming: true,
      contentBlocks: [{ type: 'text', id: '0190b1c2-asst:0', text: 'Running npm build...' }],
    } as AgentMessage;
    ...
    expect(seedStreamFromSnapshot).toHaveBeenCalledWith(agentId, inFlightAssistant, WS);
  });
  ...
  it('snapshot merge skips in-flight message already in persisted set (dedup by ID)', async () => {
    ...
    agentsApi.getConversation.mockResolvedValueOnce(conversation([finalizedAssistant]) as never);
