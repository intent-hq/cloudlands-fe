...
<div class={cn('relative inline-flex', className)}>
  ...
  {:else if state === 'needs-permission'}
    ...
    >
      ...
      <Fa
        ...
        size={21}
        class="text-background origin-center absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-0"
      />
    </span>
  {:else if state === 'attention-discussion'}
    <!-- Attention: discussion requested - amber comment bubble -->
    <span
      class="absolute -top-0.5 -right-0.75 p-0.5 bg-sidebar rounded-full flex items-center justify-center text-warning"
      style="font-size: {Math.max(8, size * 0.35)}px; line-height: 1;"
    >
  ...
  {:else if state === 'attention-blocker'}
    ...
    <span
      class="absolute -top-0.5 -right-0.75 p-0.5 bg-sidebar rounded-full flex items-center justify-center text-destructive"
      style="font-size: {Math.max(8, size * 0.35)}px; line-height: 1;"
    >
