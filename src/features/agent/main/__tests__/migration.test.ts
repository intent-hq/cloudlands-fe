...
describe('Agent Migration Integration Tests', () => {
  ...
  describe('Persistence Operations', () => {
    describe('Save Agent', () => {
      it('should successfully save an agent', async () => {
        mockBackendHandler.handleGetAgent.mockResolvedValue({
          success: true,
          agent: {
            id: BrandedIds.AgentId('agent-123'),
        ...
        mockBackendHandler.handlePersistenceSave.mockResolvedValue({
          success: true,
        });

        const request: AgentIpc.SaveRequest = {
          agentId: BrandedIds.AgentId('agent-123'),
          workspaceId: BrandedIds.WorkspaceId('workspace-123'),
        };

        const response = await adapter.saveAgent(request);

        expect(response.success).toBe(true);
        expect(mockBackendHandler.handlePersistenceSave).toHaveBeenCalled();
      });
      ...
      it('should handle save errors', async () => {
        ...
        await expect(adapter.saveAgent(request)).rejects.toThrow('Failed to save agent');
      });
    });

    describe('Load Agent', () => {
      it('should successfully load an agent', async () => {
        const mockAgent = {
          id: BrandedIds.AgentId('agent-123'),
          name: 'Saved Agent',
          workspaceId: BrandedIds.WorkspaceId('workspace-123'),
        ...
        mockBackendHandler.handlePersistenceLoad.mockResolvedValue({
          success: true,
          agent: mockAgent,
        });

        const request: AgentIpc.LoadRequest = {
          agentId: BrandedIds.AgentId('agent-123'),
          workspaceId: BrandedIds.WorkspaceId('workspace-123'),
        };

        const response = await adapter.loadAgent(request);

        expect(response.success).toBe(true);
        expect(response.agent).toEqual(mockAgent);
        expect(mockBackendHandler.handlePersistenceLoad).toHaveBeenCalledWith(
          null,
          expect.objectContaining({
            agentId: request.agentId,
            workspaceId: request.workspaceId,
      ...
      it('should handle load errors', async () => {
        mockBackendHandler.handlePersistenceLoad.mockResolvedValue({
          success: false,
          error: 'Agent not found in storage',
        });
        ...
        await expect(adapter.loadAgent(request)).rejects.toThrow('Agent not found in storage');
      });
    });

    describe('List Saved Agents', () => {
      it('should successfully list saved agents', async () => {
        const mockAgents = [
          {
            id: BrandedIds.AgentId('saved-1'),
            name: 'Saved Agent 1',
            workspaceId: BrandedIds.WorkspaceId('workspace-123'),
          },
          {
            id: BrandedIds.AgentId('saved-2'),
            name: 'Saved Agent 2',
            workspaceId: BrandedIds.WorkspaceId('workspace-123'),
          },
        ];

        mockBackendHandler.handlePersistenceList.mockResolvedValue({
          success: true,
          agents: mockAgents,
        });

        const request: AgentIpc.ListSavedRequest = {
          workspaceId: BrandedIds.WorkspaceId('workspace-123'),
        };

        const response = await adapter.listSavedAgents(request);

        expect(response.agents).toEqual(mockAgents);
        expect(mockBackendHandler.handlePersistenceList).toHaveBeenCalledWith(
          null,
          expect.objectContaining({
            workspaceId: request.workspaceId,
          }),
      ...
      it('should return empty array when no saved agents', async () => {
        mockBackendHandler.handlePersistenceList.mockResolvedValue({
          success: true,
          agents: [],
        });
