 ...
 * sees, so it also replaces the store content.
 */
 ...
 * newer rev also keeps its content. Resolves with what the store holds
 * afterwards.
 */
function applyContentSaveResult(
  ...
  if (superseded) {
    rebasePendingDrafts(key, noteId, sent, echoed, nextRev);
