...
import { resolveEmptyWindowDestination } from './utils/empty-window-destination';
 ...
 * the next tab (or the shared empty-window destination). Uses the global
 * SvelteKit `goto` that `+layout.svelte`
...
export async function navigateAwayIfViewing(workspaceId: string): Promise<void> {
  ...
  const target =
    typeof nextTabId === 'string' && nextTabId.length > 0 && nextTabId !== workspaceId
      ? `/workspace/${nextTabId}`
      : resolveEmptyWindowDestination(selectWorkspaceItems.select(appStore.state), workspaceId);
 ...
 * (or the shared empty-window destination) only when the closed workspace is
 * the one on screen.
