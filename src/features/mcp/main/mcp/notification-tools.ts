 ...
 *
 * Provides an MCP tool for emitting notification events into the workspace event bus.
...
/**
 * Tool to emit a notification event into the workspace event bus.
 */
export class EmitNotificationTool extends Tool {
  ...
  getDefinition() {
    return {
      ...
      description:
        'Emit a notification event into the workspace event bus. Used by external services to deliver notifications to subscribed agents.',
      inputSchema: {
  ...
  async execute(call: ToolCall): Promise<ToolResult> {
    try {
        ...
        try {
          ...
          if (resumability.canWake) {
            ...
            if (wakeResult.success) {
              ...
              logger.warn('Failed to wake source agent for notification, falling back to event bus only', {
                agentId: sourceAgent,
                topic,
                workspaceId: this.workspaceId,
            ...
            logger.warn('Source agent is not resumable, falling back to event bus only', {
              agentId: sourceAgent,
              topic,
              workspaceId: this.workspaceId,
          ...
          logger.warn('Error attempting to wake source agent, falling back to event bus only', {
            agentId: sourceAgent,
            topic,
            workspaceId: this.workspaceId,
      ...
      // Always emit the event on the workspace event bus (broadcast for any other subscribers)
