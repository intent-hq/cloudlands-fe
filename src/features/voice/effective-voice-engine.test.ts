...
function inputs(overrides: Partial<EffectiveVoiceEngineInputs> = {}): EffectiveVoiceEngineInputs {
  return {
    isLoading: false,
    engine: 'daemon',
...
describe('resolveEffectiveVoiceEngine', () => {
  it('honors an explicit os selection unconditionally (no silent cloud fallback)', () => {
    ...
    expect(resolveEffectiveVoiceEngine(inputs({ engine: 'os', osEngineAvailable: true }))).toBe(
      'os',
    );
  });
  ...
  it("resolves daemon when the selected provider's key is configured", () => {
    expect(
      resolveEffectiveVoiceEngine(inputs({ keyConfigured: { elevenlabs: true, openai: false } })),
    ).toBe('daemon');
  });
  ...
  it('falls back to os when the daemon key is missing and the OS engine is available', () => {
    expect(resolveEffectiveVoiceEngine(inputs({ osEngineAvailable: true }))).toBe('os');
  });
  ...
  it('never gates on unsettled state: daemon while the initial read is loading', () => {
    expect(resolveEffectiveVoiceEngine(inputs({ isLoading: true }))).toBe('daemon');
  });
});
