...
describe('FeedPanel two-line rows', () => {
  it('shows the resolved agent name on line 1 and the workspace-prefixed detail on line 2', async () => {
    render(FeedPanel);
    appStore.dispatch(
      setWorkspaceEntity(
        makeWorkspace('ws-1', 'Sidecar update', [{ id: 'a1', name: 'Implementor' }]),
      ),
    );
    appStore.dispatch(
  ...
  it('falls back to the workspace title on line 1 when no agent resolves', async () => {
    render(FeedPanel);
  ...
  it('color-codes the detail line red for failures and yellow for attention', async () => {
    render(FeedPanel);
    appStore.dispatch(
      setWorkspaceEntity(
        makeWorkspace('ws-1', 'Sidecar update', [{ id: 'a1', name: 'Implementor' }]),
    ...
    appStore.dispatch(
      hudFeedEntryReceived(
        feedEntry({
          ...
          agentId: 'a1',
          colorClass: 'warn',
          kind: 'workspace:attention-changed',
          text: 'needs input',
        }),
      ),
    );
    ...
    await waitFor(() => {
      flushSync();
      expect(screen.getByTestId('hud-feed-panel').querySelectorAll('.hud-feed-row')).toHaveLength(
        2,
      );
    });
  ...
  it('leaves the detail line uncolored for non-failure/attention rows', async () => {
    render(FeedPanel);
    appStore.dispatch(
      setWorkspaceEntity(
        makeWorkspace('ws-1', 'Sidecar update', [{ id: 'a1', name: 'Implementor' }]),
    ...
    appStore.dispatch(
      hudFeedEntryReceived(
        feedEntry({ agentId: 'a1', colorClass: 'info', kind: 'agent:completed', text: 'Done' }),
      ),
    );
    let text: HTMLElement | null = null;
    await waitFor(() => {
      flushSync();
      text = row().querySelector('.hud-feed-text');
      expect(text).toBeTruthy();
    });
    if (!text) throw new Error('Expected feed detail text');
    expect(text.style.color).toBe('');
  });
});

describe('FeedPanel agent-status chip wording', () => {
  async function chipFor(agentStatus: string): Promise<string> {
    appStore.dispatch(hudActivated());
    render(FeedPanel);
    appStore.dispatch(
      setWorkspaceEntity(
    ...
    await waitFor(() => {
      flushSync();
      chip = row().querySelector('.hud-feed-tag')?.textContent ?? '';
  ...
  it('never leaks the raw wire status into the chip or the detail line', async () => {
    render(FeedPanel);
    appStore.dispatch(
      setWorkspaceEntity(
        makeWorkspace('ws-1', 'Sidecar update', [{ id: 'a1', name: 'Implementor' }]),
...
describe('FeedPanel WORKSPACE STATUS rows', () => {
  async function statusRow(displayStatus: string): Promise<HTMLElement> {
    appStore.dispatch(hudActivated());
    render(FeedPanel);
    appStore.dispatch(setWorkspaceEntity(makeWorkspace('ws-1', 'Sidecar update')));
    appStore.dispatch(
      hudFeedEntryReceived(
        feedEntry({
          id: `evt-${displayStatus}`,
          kind: 'workspace:displayStatus-changed',
          displayStatus,
        }),
      ),
    );
    let renderedRow: HTMLElement | null = null;
    await waitFor(() => {
      flushSync();
      renderedRow = row();
      expect(renderedRow).toBeTruthy();
    });
    if (!renderedRow) throw new Error('Expected a workspace-status row');
    return renderedRow;
  }
  ...
  it('renders the localized card-state label per status — never the raw wire value', async () => {
  ...
  it('an unknown wire status renders no detail rather than leaking the raw value', async () => {
    expect((await statusRow('some_future_status')).querySelector('.hud-feed-line2')).toBeNull();
  });

  it('colors the row dot with the reported status card color token', async () => {
    ...
    for (const [wire, stateKey] of expected) {
      const dot = (await statusRow(wire)).querySelector('.hud-feed-dot') as HTMLElement;
      expect(dot.style.background, wire).toBe(cardStateColor(stateKey));
      cleanup();
    }
  });

  it('other kinds keep their feed color-class dot', async () => {
    render(FeedPanel);
    appStore.dispatch(setWorkspaceEntity(makeWorkspace('ws-1', 'Sidecar update')));
    appStore.dispatch(
    ...
    await waitFor(() => {
      flushSync();
      dot = row().querySelector('.hud-feed-dot');
      expect(dot).toBeTruthy();
    });
    if (!dot) throw new Error('Expected a feed dot');
