...
<script lang="ts">
  ...
  function getSearchContentElements(searchRoot: ParentNode): HTMLElement[] {
    const candidates = Array.from(
      searchRoot.querySelectorAll<HTMLElement>(SEARCH_CONTENT_SELECTOR),
    );
  ...
  function scrollElementIntoContainer(element: HTMLElement, container: HTMLElement) {
    ...
    if (elementRect.left < containerRect.left + SEARCH_SCROLL_MARGIN_PX) {
      ...
      nextLeft = Math.max(
        0,
        targetLeft - container.clientWidth + elementRect.width + SEARCH_SCROLL_MARGIN_PX,
      );
    }
    ...
    >
      ...
      {#if diffData}
        <div
          ...
          class="pure-diff-container"
          style:--diffs-font-family={$codeFontFamilyCSS}
        ></div>
      {:else}
