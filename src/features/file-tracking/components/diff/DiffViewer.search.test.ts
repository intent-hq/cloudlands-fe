...
describe('DiffViewer search', () => {
  ...
  it('debounces non-empty input before walking and highlighting the diff DOM', async () => {
    ...
    await fireEvent.input(screen.getByPlaceholderText('Find in diff...'), {
      target: { value: 'repeat' },
    });
    await tick();
  ...
  it('refreshes highlights after a structural rerender replaces the diff shadow DOM', async () => {
    vi.useFakeTimers();
    const view = render(DiffViewer, {
      props: { oldContent: '', newContent: 'repeat', viewMode: 'unified' },
    });
  ...
  it('renders the find bar as a sticky row inside the scrollable diff content', async () => {
    const { container } = render(DiffViewer, {
      props: { oldContent: '', newContent: 'repeat', maxHeight: '120px' },
    });
  ...
  it('scrolls the owning diff content container when navigating to the selected match', async () => {
    ...
    diffContent.getBoundingClientRect = () =>
      ({ top: 0, bottom: 100, left: 0, right: 500, width: 500, height: 100 }) as DOMRect;
    ...
    highlights[1].getBoundingClientRect = () =>
      ({ top: 500, bottom: 510, left: 0, right: 10, width: 10, height: 10 }) as DOMRect;

    await fireEvent.click(screen.getByTitle('Next match (Enter)'));

    expect(screen.getByText('2 / 4')).toBeTruthy();
    expect(diffContent.scrollTo).toHaveBeenCalledWith(
      expect.objectContaining({ top: 455, behavior: 'smooth' }),
    );
  });
});
