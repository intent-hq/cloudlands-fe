<script lang="ts">
  ...
  function chunksToUnifiedPatch(chunk: DiffChunk, path: string): string {
    const a = `a/${path}`;
    const b = `b/${path}`;
    const lines: string[] = [`diff --git ${a} ${b}`, `--- ${a}`, `+++ ${b}`];
    for (const hunk of chunk.chunks) {
      lines.push(`@@ -${hunk.oldStart},${hunk.oldLines} +${hunk.newStart},${hunk.newLines} @@`);
      for (const line of hunk.lines) {
        const prefix =
          line.type === LineType.Addition ? '+' : line.type === LineType.Deletion ? '-' : ' ';
  ...
  {:else}
    <div class="diff-wrapper">
      ...
      {#if selectedLines}
        ...
        {@const lineSet =
          selectedLines.side === 'additions' ? changedLines.additions : changedLines.deletions}
