<script lang="ts">
  /**
   * Models card (design card 2, 360×640, re-themed from "Providers" per
   * Spec D3): share bar + ranked list of the top 4 normalized models by
   * total tokens with amount + %, and a MOST USED callout. Degrades
   * gracefully below 4 models; a zero-data period renders an empty state
   * without NaN.
   */
  ...
  const ranked = $derived(rankModels(data?.byModel ?? []));
  const top = $derived(ranked[0] ?? null);
</script>

<div class="card" data-stats-card="models">
  <div class="head">
    <div class="head-brand">
      <Logo width={26} />
      <span class="head-title">MODELS</span>
    </div>
  ...
  <div class="body">
    <div class="bar">
      {#each ranked as m, i (m.model)}
        <span
          class="bar-seg"
          style="width:{(m.share * 100).toFixed(1)}%;background:{MODEL_BAR_COLORS[
    ...
    <div class="list">
      ...
      {:else}
        <div class="row row-last row-empty">No model usage in this period</div>
      {/each}
    </div>
    <div class="callout">
