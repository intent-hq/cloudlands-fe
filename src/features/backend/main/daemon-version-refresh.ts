 ...
 * version the transport payload keeps broadcasting the stale
 * `daemonVersion`/`versionMismatch`. The `client.hello` handshake re-runs on
 * every (re)connect and its result carries the live daemon version
 * (`server.version`, PROTOCOL §5.17) — this helper recomputes the info from
 * it. Extracted from `backend.ipc.ts` so the gating + parse logic is testable
 * without the Electron module graph.
 *
 ...
 */
export function computeDaemonVersionRefresh(args: DaemonVersionRefreshArgs): DaemonVersionInfo | null {
