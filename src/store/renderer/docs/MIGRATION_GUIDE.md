...
7. Add app saga functions to the ordered `sagas` array in `src/store/renderer/sagas.ts`; root startup runs them through `startAllAppSagas(store)` after Store initialization.
