...
describe('statsReducer', () => {
  ...
  it('records the selected mode/period and enters loading on request', () => {
    const next = statsReducer(initialState, loadUsageStatsRequested('month', '2026-07', -420));
  ...
  it('stores the daemon result for the matching selection', () => {
    const requested = statsReducer(initialState, loadUsageStatsRequested('month', '2026-07', 0));
  ...
  it('discards stale replies after the selection changed', () => {
    const requested = statsReducer(initialState, loadUsageStatsRequested('year', '2026', 0));
  ...
  it('surfaces fetch failures for the matching selection', () => {
    const requested = statsReducer(initialState, loadUsageStatsRequested('month', '2026-07', 0));
    const failed = statsReducer(requested, usageStatsFailed('month', '2026-07', 'uds boom'));
