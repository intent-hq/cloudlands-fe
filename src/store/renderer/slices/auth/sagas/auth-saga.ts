...
import { selectHasShownGitCredentialsModalForWorkspace } from "$store/renderer/slices/global-modals/global-modals-selectors";
...
export function* watchGitAuthRequiredSaga() {
  yield* takeEveryFromElectronChannel<GitAuthRequiredEvent>("git:auth-required", function* (data) {
      const hasShownForWorkspace = data.workspaceId
        ? yield* selectHasShownGitCredentialsModalForWorkspace.effect(data.workspaceId)
        : false;
