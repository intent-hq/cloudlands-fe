...
import {
  ...
  initialState as associationsInitialState,
...
describe('fallback plan card wire pipeline (mock-BE)', () => {
  ...
  it('drives the rendered fallback list from a PROTOCOL-shaped task.list response', async () => {
    ...
    // Associations hydrated with no links — the legitimate Coordinator/root
    // signal (the selector returns [] until hydration, monorepo#3249).
    const taskAgentAssociations = taskAgentAssociationsReducer(
      associationsInitialState,
      hydrateTaskAgentAssociations('ws-1', {}),
    );
    const state = { workspaceTasks, taskAgentAssociations } as unknown as StoreState;
  ...
  it('drives the delegated single-task view from task.list + task.listAgentLinks responses', async () => {
    ...
    // Fold the wire response through the real associations reducer so the
    // hydrated flag is set exactly the way production hydration sets it.
    const taskAgentAssociations = taskAgentAssociationsReducer(
      associationsInitialState,
      hydrateTaskAgentAssociations('ws-1', linksByNoteId),
    );
    const state = { workspaceTasks, taskAgentAssociations } as unknown as StoreState;
