...
describe('workspace-tasks selectors', () => {
  ...
  describe('selectFallbackPlanTasksForAgent (fallback plan card, monorepo#3249)', () => {
    const withAssociations = (
      ...
      ({
        ...(base as object),
        taskAgentAssociations: { byWorkspaceId: { [WS]: { byNoteId, hydrated } } },
      }) as unknown as StoreState;
    ...
    it("shows only the delegated agent's linked task when associations exist", () => {
      ...
      expect(selectFallbackPlanTasksForAgent.select(state, WS, 'agent-1').map((t) => t.id)).toEqual(
        ['t2'],
      );
    });
    ...
    it('returns [] until agent-link associations have hydrated (no Coordinator flash)', () => {
      const base = stateWith([{ ...makeTask('t1', 'not_started'), specLinked: true }]);

      // Slice absent entirely (before any task.listAgentLinks resolves).
      expect(selectFallbackPlanTasksForAgent.select(base, WS, 'agent-1')).toEqual([]);

      // Workspace entry exists but is not yet hydrated (e.g. an eager
      // task:agent-linked event arrived before the hydrate read).
      const unhydrated = withAssociations(base, {}, false);
      expect(selectFallbackPlanTasksForAgent.select(unhydrated, WS, 'agent-1')).toEqual([]);

      // Hydrated with no links → legitimate root/Coordinator view.
      const hydrated = withAssociations(base, {});
      expect(
        selectFallbackPlanTasksForAgent.select(hydrated, WS, 'agent-1').map((t) => t.id),
      ).toEqual(['t1']);
    });
  });
});
