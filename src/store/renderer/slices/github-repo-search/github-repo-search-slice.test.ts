...
describe('githubRepoSearchReducer', () => {
  it('returns the initial state', () => {
    expect(githubRepoSearchReducer(undefined, { type: '@@INIT' })).toEqual(initialState);
  });
  ...
  it('records the active query and flips loading while searching', () => {
    ...
    const next = githubRepoSearchReducer(previous, setGithubRepoSearchLoading('svelte'));
  ...
  it('stores results as a Collection keyed by owner/name', () => {
    ...
    const repos = [mockRepo('sveltejs', 'svelte'), mockRepo('sveltejs', 'kit')];

    const next = githubRepoSearchReducer(loading, setGithubRepoSearchResults('svelte', repos));
  ...
  it('records an error, clears loading, and drops previous results', () => {
    const populated = {
      ...initialState,
      results: createCollection<GithubRepoItem, 'id'>('id', [mockRepo('sveltejs', 'svelte')]),
      loading: true,
      lastQuery: 'svelte',
    };
  ...
  it('resets to initial state on clear', () => {
    ...
    expect(githubRepoSearchReducer(populated, clearGithubRepoSearch())).toEqual(initialState);
  });
});
