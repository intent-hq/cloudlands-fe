...
function asStoreState(ft: ReturnType<typeof fileTrackingReducer>): StoreState {
  return { changes: ft } as unknown as StoreState;
}
...
describe('fileTrackingReducer', () => {
  ...
  it('appendOlderCommits deduplicates by hash', () => {
    ...
    state = fileTrackingReducer(
      state,
      appendOlderCommits(WS, [mockCommit('h1'), mockCommit('h2')]),
    );
  ...
  it('updateAgentStats overwrites existing agent stats', () => {
    const stats1: LineChangeStats = {
      additions: 3,
      deletions: 1,
      timestamp: '2025-01-01T00:00:00Z',
    };
    const stats2: LineChangeStats = {
      additions: 10,
      deletions: 5,
...
describe('changes selectors', () => {
  ...
  it('selectAllAgentStats returns all agent stats', () => {
    let state = fileTrackingReducer(
      initialState,
      updateAgentStats('a1', { additions: 1, deletions: 0, timestamp: '' }),
    );
    state = fileTrackingReducer(
      state,
      updateAgentStats('a2', { additions: 2, deletions: 0, timestamp: '' }),
    );
  ...
  it('selectShouldRequestAgentLineStats dedupes cache and in-flight state while allowing force refresh', () => {
    ...
    expect(
      selectShouldRequestAgentLineStats.select(asStoreState(initialState), 'agent-1', false),
    ).toBe(true);
    expect(selectShouldRequestAgentLineStats.select(asStoreState(cached), 'agent-1', false)).toBe(
      false,
    );
    expect(selectShouldRequestAgentLineStats.select(asStoreState(cached), 'agent-1', true)).toBe(
      true,
    );
    expect(selectShouldRequestAgentLineStats.select(asStoreState(inFlight), 'agent-1', true)).toBe(
      false,
    );
  });
});
