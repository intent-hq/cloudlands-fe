...
export const selectMainPanelView: AppSelector<MainPanelViewState | null> = store.createSelector(
...
export const selectFileTrackingLoading: AppSelector<boolean, [wsId: string]> = store.createSelector(
...
export const selectCurrentChanges: AppSelector<TrackedChange[], [wsId: string | null | undefined]> =
  store.createSelector((state, wsId: string | null | undefined): TrackedChange[] => {
    if (!wsId) return [];
...
export const selectCurrentCommits: AppSelector<CommitInfo[], [wsId: string | null | undefined]> =
  store.createSelector((state, wsId: string | null | undefined): CommitInfo[] => {
    if (!wsId) return [];
...
> = store.createSelector((state, agentId: string, forceRefresh = false): boolean => {
...
export const selectSidebarCommitWhenReady: AppSelector<boolean, [workspaceId: string]> =
  store.createSelector(
    (state, workspaceId: string): boolean =>
      getWs(state, workspaceId).acceptChanges.commitWhenReady,
  );
