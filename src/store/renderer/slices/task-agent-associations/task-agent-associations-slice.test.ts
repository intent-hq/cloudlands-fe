...
describe('taskAgentAssociationsReducer', () => {
  it('hydrates associations by workspace and note', () => {
    ...
    expect(next.byWorkspaceId['ws-1'].hydrated).toBe(true);
  });

  it('marks a workspace hydrated even when the daemon returns no links, but not before', () => {
    const beforeHydration = taskAgentAssociationsReducer(
      initialState,
      addTaskAgentAssociation('ws-1', 'note-1', association),
    );
    expect(beforeHydration.byWorkspaceId['ws-1'].hydrated).toBe(false);
    ...
    expect(next.byWorkspaceId['ws-1'].hydrated).toBe(true);
    expect(next.byWorkspaceId['ws-1'].byNoteId).toEqual({});
  });

  it('preserves the hydrated flag across add/remove edits', () => {
    const hydrated = taskAgentAssociationsReducer(
      initialState,
      hydrateTaskAgentAssociations('ws-1', {}),
    );
    ...
    expect(added.byWorkspaceId['ws-1'].hydrated).toBe(true);
    expect(removed.byWorkspaceId['ws-1'].hydrated).toBe(true);
  });
