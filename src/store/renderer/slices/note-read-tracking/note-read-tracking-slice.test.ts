...
describe('noteReadTrackingReducer', () => {
  ...
  describe('computeUnreadNotesSuccess', () => {
    it('should replace unread IDs and clear loading', () => {
      const prev: NoteReadTrackingState = {
        ...initialState,
        unreadNoteIds: { stale: true },
        isLoading: true,
      };
      const state = noteReadTrackingReducer(prev, computeUnreadNotesSuccess(['note-1', 'note-2']));
      expect(state.unreadNoteIds).toEqual({ 'note-1': true, 'note-2': true });
      expect(state.isLoading).toBe(false);
    });

    it('should clear unread IDs when the computed set is empty', () => {
      const prev: NoteReadTrackingState = {
        ...initialState,
        unreadNoteIds: { 'note-1': true },
      };
      const state = noteReadTrackingReducer(prev, computeUnreadNotesSuccess([]));
  ...
  describe('setLoading', () => {
    it('should set the loading flag', () => {
      ...
      expect(noteReadTrackingReducer(state, setLoading(false)).isLoading).toBe(false);
    });
  });
