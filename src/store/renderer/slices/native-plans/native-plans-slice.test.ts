...
describe('nativePlansReducer', () => {
  ...
  it('applyNativePlanUpdated stores entries keyed by session id', () => {
    ...
    expect(getItems(state.bySessionId['acp-1'].entries)).toEqual([entry('e1', 'pending')]);
  });
  ...
  it('applyNativePlanUpdated replaces prior entries for the session', () => {
    ...
    const items = getItems(state.bySessionId['acp-1'].entries);
    expect(items.map((e) => e.id)).toEqual(['e1', 'e2']);
