...
import {
  ...
  selectNativePlanEntries,
} from "./native-plans-selectors";
import type { NativePlanEntry } from "./native-plans-types";

const entries: NativePlanEntry[] = [{ id: "e1", title: "Entry", status: "pending" }];

function stateWith(
  bySessionId: Record<string, { entries: NativePlanEntry[] }>,
...
describe("native-plans selectors", () => {
  it("selectNativePlanEntries returns entries for a session and [] otherwise", () => {
    ...
    expect(selectNativePlanEntries.select(state, "acp-1")).toEqual(entries);
