...
describe('agentEventsIpcSaga', () => {
  ...
  it('shows exact auth and plan toasts and routes the terminal action', async () => {
    ...
    mocks.callbacks['agent:auth-required']({ workspaceId: 'ws-1', agentId: 'a-1', isRemote: true, host: 'host', message: 'Sign in' });
    mocks.callbacks['agent:plan-required']({ workspaceId: 'ws-1', agentId: 'a-1', message: 'Upgrade', helpUrl: 'https://help' });
    await settle();

    const authOptions = mocks.warning.mock.calls[0][1];
    expect(mocks.warning.mock.calls[0]).toEqual(['Agent Authentication Required', {
      description: 'Sign in', duration: 15_000,
      action: { label: 'Open Terminal', onClick: authOptions.action.onClick },
    }]);
    expect(mocks.error.mock.calls[0]).toEqual(['Intent: Plan Upgrade Required', {
      description: 'Upgrade', duration: 20_000,
    }]);
  ...
  it('does not route an auth toast without a workspace and swallows toast failures', async () => {
    mocks.warning.mockImplementationOnce(() => { throw new Error('unavailable'); });
    const task = runSaga({ dispatch: vi.fn(), getState: () => ({}) }, agentEventsIpcSaga);
  ...
  it('registers all channels and unregisters them on cancellation', async () => {
    const task = runSaga({ dispatch: vi.fn(), getState: () => ({}) }, agentEventsIpcSaga);
    expect(mocks.on.mock.calls.map(([channel]) => channel)).toEqual(['agent:auth-required', 'agent:plan-required']);
    task.cancel();
    await task.toPromise();
