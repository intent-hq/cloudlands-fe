...
import type {
  WorkspaceCreateProgressState,
} from "./workspace-create-progress-types";
 ...
 */
export const beginWorkspaceCreateProgress = createAction<[
  progressId: string,
...
export const workspaceCreateProgressDone = createAction<[
  progressId: string,
  outcome: { ok: boolean; error?: string; errorCode?: string },
]>("workspaceCreateProgress/done");

/** Drop the entry once the create settles (success or failure). */
export const clearWorkspaceCreateProgress = createAction<[
  progressId: string,
...
workspaceCreateProgressReducer.with(workspaceCreateProgressDone, (state, { payload: [progressId, outcome] }) => {
  const entry = state.byProgressId[progressId];
...
workspaceCreateProgressReducer.with(clearWorkspaceCreateProgress, (state, { payload: [progressId] }) => {
