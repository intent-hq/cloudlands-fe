...
describe('hardwareConsoleReducer', () => {
  ...
  it('sets the prompt picker limit, clamped to [1, 12]', () => {
    ...
    expect(hardwareConsoleReducer(initialState, setPromptPickerLimit(99)).promptPickerLimit).toBe(
      12,
    );
  ...
  it('opens, retargets, and closes the radial picker overlay', () => {
    const opened = hardwareConsoleReducer(initialState, radialPromptPickerOpened(['a', 'b'], 1));
  ...
  it('shows and hides the action-key HUD label', () => {
    const shown = hardwareConsoleReducer(initialState, actionHudShown('Cycle in-progress agents'));
  ...
  it("sets one family's cycle scope and no-ops on repeats", () => {
    const state = hardwareConsoleReducer(initialState, setCycleScope('cycle-idle-agents', 'all'));
    ...
    expect(hardwareConsoleReducer(initialState, setCycleScope('cycle-failed-agents', 'all'))).toBe(
      initialState,
    );
  });
});
