...
vi.mock('$store/renderer/slices/agent-session/agent-session-slice', () => {
  ...
  const agentSessionStopChatRequested = vi.fn((agentId: string, options?: unknown) => ({
    type: 'agentSessions/stopChatRequested',
    asyncActionType: 'agentSessions/stopChat',
    payload: [agentId, options],
    promise: Promise.resolve().then(() => mockStopChat(agentId, options)),
    success: vi.fn((response: unknown) => ({
      type: 'agentSessions/stopChatRequested_SUCCESS',
      payload: { request: [agentId, options], response },
    })),
    failure: vi.fn((error: string) => ({
      type: 'agentSessions/stopChatRequested_FAILURE',
      payload: { request: [agentId, options], error },
    })),
  }));
  const addMessage = vi.fn((agentId: string, message: unknown) => ({
  ...
  addMessage.type = 'agentSessions/addMessage';
  addMessage.toString = () => 'agentSessions/addMessage';
  return {
...
import { sendMessage, chatSendStarted, chatSendFailed } from '../chat-state-slice';
import {
...
describe('send-message-saga', () => {
  ...
  describe('immediate loading state (regression)', () => {
    ...
    it('dispatches chatSendStarted before clearChatDraft and uncheckAllSelections', async () => {
      ...
      const sendStartedIdx = dispatched.findIndex((a) => a.type === chatSendStarted.type);
      const clearDraftIdx = dispatched.findIndex((a) => a.type === 'transientUi/clearChatDraft');
  ...
  describe('rebind timeout resets loading state', () => {
    it('dispatches chatSendFailed after chatSendStarted when rebind times out', async () => {
      ...
      const sendStartedIdx = dispatched.findIndex((a) => a.type === chatSendStarted.type);
      const sendFailedIdx = dispatched.findIndex((a) => a.type === chatSendFailed.type);
  ...
  describe('agent-session failure side effects are not duplicated', () => {
    it('does not dispatch chatSendFailed locally when the core send action rejects', async () => {
      ...
      const sendFailedAction = dispatched.find((a) => a.type === chatSendFailed.type);
  ...
  describe('workspace reinit failure resets loading state', () => {
    it('dispatches chatSendFailed when workspace reinit fails', async () => {
      ...
      const sendStarted = dispatched.find((a) => a.type === chatSendStarted.type);
  ...
  describe('queue path does not dispatch chatSendStarted', () => {
    it('takes queue path when agent is streaming and does not dispatch chatSendStarted', async () => {
      ...
      expect(mockQueueMessage).toHaveBeenCalledWith(AGENT_ID, 'Hello world', [], undefined, WS_ID);
    });
  ...
  describe('concurrent send guard', () => {
    it('queues a second sendMessage for the same agent while the first is in-flight', async () => {
      ...
      const sendStartedActions = dispatched.filter((a) => a.type === chatSendStarted.type);
      ...
      // Successful queueing clears the chat draft
      expect(dispatched.find((a) => a.type === 'transientUi/clearChatDraft')).toBeDefined();
  ...
  describe('happy path', () => {
    it('dispatches chatSendStarted, calls agent-session send trigger, dispatches clearChatDraft', async () => {
      ...
      const clearDraftAction = dispatched.find((a) => a.type === 'transientUi/clearChatDraft');
  ...
  // ========================================================================
  // Optimistic user message (rendered from agent-session state)
  // ========================================================================
  describe('optimistic user message', () => {
    it('dispatches an optimistic addMessage before the send trigger with matching userAppMessageId', async () => {
      const { dispatched } = await runSendMessageSaga();

      const addIdx = dispatched.findIndex((a) => a.type === 'agentSessions/addMessage');
      const sendRequestedIdx = dispatched.findIndex(
        (a) => a.type === 'agentSessions/sendMessageRequested',
      );
      expect(addIdx).toBeGreaterThanOrEqual(0);
      expect(sendRequestedIdx).toBeGreaterThan(addIdx);

      const [addAgentId, optimisticMessage] = dispatched[addIdx].payload;
      expect(addAgentId).toBe(AGENT_ID);
      expect(optimisticMessage).toMatchObject({
        role: 'user',
        contentBlocks: [{ type: 'text', text: 'Hello world' }],
      });
      expect(optimisticMessage.appMessageId).toMatch(/^app_msg_/);
      expect(optimisticMessage.id).toMatch(/^optimistic_/);
      expect(typeof optimisticMessage.timestamp).toBe('string');

      // The send options must thread the SAME ids so the canonical message
      // merges with the optimistic one via appMessageId dedup.
      const options = mockSendMessage.mock.calls[0][3];
      expect(options.userAppMessageId).toBe(optimisticMessage.appMessageId);
      expect(options.optimisticMessageId).toBe(optimisticMessage.id);
    });

    it('does not dispatch an optimistic addMessage on the queue path', async () => {
      mockSelectAgentIsResponding.mockReturnValue(true);

      const { dispatched } = await runSendMessageSaga();

      expect(mockQueueMessage).toHaveBeenCalled();
      expect(dispatched.find((a) => a.type === 'agentSessions/addMessage')).toBeUndefined();
    });
  });
  ...
  describe('MessageGuardError handling (idempotency fix)', () => {
    it('does not duplicate chatSendFailed when sendMessage throws MessageGuardError', async () => {
      ...
      mockSendMessage.mockRejectedValue(new MockMessageGuardError('Duplicate message detected'));
  ...
  describe('skipQueueCheck forces send path (root cause #5)', () => {
    ...
    it('forceSubmit stops the active response before sending', async () => {
      ...
      expect(mockStopChat).toHaveBeenCalledWith(AGENT_ID, undefined);
      expect(mockSendMessage).toHaveBeenCalled();
    });
  });
  ...
  describe('send now with image blocks (regression)', () => {
    ...
    it('does not replay a queued message when saga-owned queue removal fails', async () => {
      ...
      expect(
        dispatched.find((a) => a.type === removeQueuedMessageFromAgentQueue.type),
      ).toBeUndefined();
      expect(dispatched.find((a) => a.type === chatSendStarted.type)).toBeUndefined();
      expect(mockSendMessage).not.toHaveBeenCalled();
    });
    ...
    it('passes serializedContextItems through unchanged when both it and imageBlocks are present (no image duplication)', async () => {
      ...
      const imageBlocks = [{ type: 'image' as const, data: 'base64data1', mimeType: 'image/png' }];
    ...
    it('passes serializedContextItems through unchanged even when it mixes a file and an image alongside imageBlocks', async () => {
      ...
      const imageBlocks = [{ type: 'image' as const, data: 'imgdata', mimeType: 'image/png' }];
