...
function harness() {
  ...
  return { channel, dispatch, task, messages: () => agentSessions.byAgentId[AGENT]?.messages ?? [] };
}
...
describe('agentStreamSaga', () => {
  it('updates rich tool blocks in place, preserves order, and finalizes interrupted streams', async () => {
    ...
    run.channel.put(agentStreamUpdateReceived({
      ...
      eventType: 'started', assistantMessageId: 'msg-1', assistantAppMessageId: 'app-1',
      timestamp: 1, contentBlocks: [{ type: 'text', text: '' }],
    }));
    run.channel.put(agentStreamUpdateReceived({
      agentId: AGENT, workspaceId: WS, handlerSessionId: AGENT, source: 'sendMessage',
      eventType: 'content-blocks', assistantMessageId: 'msg-1', assistantAppMessageId: 'app-1',
      contentBlocks: [
        { type: 'text', text: 'hello' },
        { type: 'tool_use', id: 'tool-1', name: 'read', input: { path: 'first' }, toolCallId: 'call-1' },
      ],
    }));
    run.channel.put(agentStreamUpdateReceived({
      agentId: AGENT, workspaceId: WS, handlerSessionId: AGENT, source: 'sendMessage',
      ...
      contentBlocks: [
        ...
        { type: 'tool_use', id: 'tool-1', name: 'read', input: { path: 'second' }, toolCallId: 'call-1' },
        { type: 'tool_result', tool_use_id: 'tool-1', output: 'done' },
      ],
    }));
    run.channel.put(agentStreamUpdateReceived({
      agentId: AGENT, workspaceId: WS, handlerSessionId: AGENT, source: 'sendMessage',
      eventType: 'complete', assistantMessageId: 'msg-1', assistantAppMessageId: 'app-1',
      stopReason: 'interrupted', contentBlocks: [
        { type: 'text', text: 'hello world' },
        { type: 'tool_use', id: 'tool-1', name: 'read', input: { path: 'second' }, toolCallId: 'call-1' },
        { type: 'tool_result', tool_use_id: 'tool-1', output: 'done' },
      ],
    ...
    expect(run.messages()[0]?.contentBlocks).toEqual([
      { type: 'text', text: 'hello world' },
      {
        ...
        input: { path: 'second' }, toolCallId: 'call-1',
      },
      { type: 'tool_result', tool_use_id: 'tool-1', output: 'done' },
    ]);
    expect(run.messages()[0]).toEqual(expect.objectContaining({
      id: 'msg-1', appMessageId: 'app-1', isStreaming: false, streamingComplete: true,
  ...
  it('isolates malformed events and still applies the following terminal event', async () => {
    const run = harness();
    run.channel.put(agentStreamUpdateReceived({
      agentId: AGENT, workspaceId: WS, handlerSessionId: AGENT, source: 'restored',
      eventType: 'started', assistantMessageId: 'bad', timestamp: Number.NaN,
    }));
    run.channel.put(agentStreamUpdateReceived({
      agentId: AGENT, workspaceId: WS, handlerSessionId: AGENT, source: 'restored',
      eventType: 'complete', assistantMessageId: 'good', timestamp: 2,
      contentBlocks: [{ type: 'text', text: 'final' }],
    }));
    await settle();

    expect(run.messages()).toHaveLength(1);
    expect(run.messages()[0]).toMatchObject({ id: 'good', isStreaming: false, streamingComplete: true });
    expect(run.dispatch.mock.calls.some(([action]) => action.type === 'chatState/streamCompleted')).toBe(true);
    run.task.cancel();
    await run.task.toPromise();
  });
  ...
  it.each([
    ...
    run.channel.put(agentStreamUpdateReceived({
      agentId: AGENT, workspaceId: WS, handlerSessionId: AGENT, source: 'restored',
      eventType: 'started', assistantMessageId: `msg-${eventType}`,
      contentBlocks: [{ type: 'text', text: 'partial' }],
    }));
    run.channel.put(agentStreamUpdateReceived({
      agentId: AGENT, workspaceId: WS, handlerSessionId: AGENT, source: 'restored',
      eventType, assistantMessageId: `msg-${eventType}`,
      error: eventType === 'error' ? 'failed' : undefined,
    }));
    await settle();

    expect(run.messages()[0]).toEqual(expect.objectContaining({
      id: `msg-${eventType}`, isStreaming: false, streamingComplete: true,
      contentBlocks: [{ type: 'text', text: 'partial' }],
    }));
  ...
  it('drains a terminal event already buffered when cancellation starts', () => {
    ...
    const started = agentStreamUpdateReceived({
      agentId: AGENT, workspaceId: WS, handlerSessionId: AGENT, source: 'restored',
      eventType: 'started', assistantMessageId: 'msg-cancelled',
      contentBlocks: [{ type: 'text', text: 'partial' }],
    });
    generator.next(started);
    const terminal = agentStreamUpdateReceived({
      agentId: AGENT, workspaceId: WS, handlerSessionId: AGENT, source: 'restored',
      eventType: 'complete', assistantMessageId: 'msg-cancelled',
