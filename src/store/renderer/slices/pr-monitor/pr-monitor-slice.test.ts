...
describe("prMonitorReducer", () => {
  it("starts with no workspaces", () => {
    expect(prMonitorReducer(undefined, { type: "@@INIT" })).toEqual(initialState);
  });
  ...
  it("prMonitorsUpdated stores lastSnapshot from the prMonitor.list wire shape (§6.9)", () => {
    ...
    const state = prMonitorReducer(
      initialState,
      prMonitorsUpdated("ws-1", [makeMonitor({ lastSnapshot, title: "Fix widget", url: "https://github.com/acme/widgets/pull/42" })]),
    );
