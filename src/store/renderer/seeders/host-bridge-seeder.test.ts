...
describe('host-bridge-seeder', () => {
  ...
  describe('editor-open intents → daemon host.openInEditor (PROTOCOL §5.14)', () => {
    ...
    it('jetbrains:open rejects visibly when no JetBrains IDE is installed', async () => {
      ...
      await expect(mockInvoke('jetbrains:open', '/repo')).rejects.toThrow(/No JetBrains IDE found/);
  ...
  describe('external-editors:open-with-other → real Electron preload bridge (FE-served, PROTOCOL §5.14 host.pickApplication locus)', () => {
    ...
    it('forwards { path } to window.electronAPI.invoke and returns the bridge response verbatim when a preload bridge exists', async () => {
      const bridgeInvoke = vi.fn().mockResolvedValue({ success: true, appName: 'Sublime Text' });
      stashedElectronApi = (window as any).electronAPI;
  ...
  describe('shell:openExternal → openExternalUrl (FE-served, PROTOCOL §5.14 reverse-RPC locus)', () => {
    ...
    it('rejects a missing url or a non-http(s) scheme without opening anything', async () => {
      ...
      ).rejects.toThrow('Refusing to open URL externally (scheme not allowed): javascript:');
      await expect(mockInvoke('shell:openExternal', { url: 'file:///etc/passwd' })).rejects.toThrow(
        'Refusing to open URL externally (scheme not allowed): file:',
      );
    ...
    it('allows only the exact allowlisted System Settings deep link, not the whole scheme', async () => {
      // BROWSER_PROTOCOLS.EXTERNAL_EXACT is a full-string allowlist: the
      // hardcoded Input Monitoring URL opens, but any other
      // x-apple.systempreferences: URL (e.g. daemon/agent-originated) rejects.
      removeElectronApi();
      const openSpy = vi.spyOn(window, 'open').mockReturnValue(null);
      const clickSpy = vi.spyOn(HTMLAnchorElement.prototype, 'click').mockImplementation(() => {});
      ...
      const response = await mockInvoke('shell:openExternal', { url: allowlisted });
      expect(response).toEqual({ success: true });

      await expect(
        mockInvoke('shell:openExternal', {
          url: 'x-apple.systempreferences:com.apple.preference.security?Privacy_Camera',
        }),
      ).rejects.toThrow('Refusing to open URL externally (scheme not allowed):');
      clickSpy.mockRestore();
      openSpy.mockRestore();
      restoreElectronApi();
    });
  ...
  describe('system:execute-command → daemon host.exec (PROTOCOL §5.14)', () => {
    ...
    it('treats an empty-string cwd as absent (never armed the guard; blank cwd stays off the wire, hostExec parity)', async () => {
      ...
      const response = await mockInvoke<{ success: boolean }>(IPC_CHANNELS.SYSTEM.EXECUTE_COMMAND, {
        command: 'git --version',
        cwd: '',
      });
...
describe('misc-ui-events-seeder window:open-new bridge', () => {
  ...
  it('opens the route as a new browsing context off the current origin when no preload bridge exists', async () => {
    ...
    expect(openSpy).toHaveBeenCalledWith(`${window.location.origin}/workspace/ws-1`, '_blank');
    expect(fakeWindow.opener).toBeNull();
    expect(response).toEqual({ success: true });
    openSpy.mockRestore();
    restoreElectronApi();
