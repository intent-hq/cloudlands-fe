...
describe('agents-seeder', () => {
  ...
  describe('preserve-messages merge', () => {
    it('preserves existing messages when AgentLite returns messages: []', async () => {
      ...
      const { seedMockStore } = await import('../mock-bootstrap');
      await seedMockStore(store, appClient, WORKSPACE_ID);
  ...
  describe('pending-deletion guard', () => {
    ...
    it("filters agents carrying the daemon's pendingDeleteAt deadline from the seeded list", async () => {
      ...
      const doomed: AgentSession = {
        ...
        messages: [],
        status: 'idle',
        pendingDeleteAt: '2026-08-11T00:00:15.000Z',
      };
      const kept: AgentSession = {
        id: KEPT,
        workspaceId: WORKSPACE_ID,
  ...
  describe('direct merge logic', () => {
    it('store structure verification', () => {
      ...
      const testSession: AgentSession = {
        ...
        isBackground: false,
        messages: [
          { id: 'msg_test_1', role: 'user', content: 'Test', createdAt: new Date().toISOString() },
        ],
        status: 'waiting' as const,
    ...
    it('manual merge preserves messages', () => {
      ...
      const merged =
        agentLite.messages.length === 0 && existing && existing.messages.length > 0
