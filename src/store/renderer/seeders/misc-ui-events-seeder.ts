...
// envelope and a rejection into `goto(route)`.
//
// When the real Electron preload bridge exists, the invoke is forwarded to
// the main-process WINDOW.OPEN_NEW handler (createAppWindow in
// system.ipc.ts) — the funnel that owns the HUD `/hud` singleton (reuse +
// focus instead of a second window). Bridging to `window.open` here used to
// BYPASS that funnel: the popup went through webview-security's
// setWindowOpenHandler instead, spawning a fresh window per click (the
// sidebar HUD button regression). Same forward-verbatim pattern as
// auto-update-bridge-seeder. A `{ success:false }` envelope is re-thrown —
// a resolved failure would strand the `.catch`-only callers on a silent
// no-op (same "a throw is the honest terminal state" idiom as the
// settings-legacy-bridge feature-codes gate).
//
// Without the bridge (browser build), the route opens as a new browsing
// context; `/hud` uses a NAMED target so repeated opens reuse the same
// HUD tab/window (the browser's closest singleton equivalent) while other
// routes keep `_blank`. A refused open THROWS for the same reason as above.
// Registered at import time so the affordance works from the very first
// render.
