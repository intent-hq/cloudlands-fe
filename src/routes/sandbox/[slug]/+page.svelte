<script lang="ts">
  ...
  import CatalogSystemPage from '$lib/component-catalog/CatalogSystemPage.svelte';
  ...
  async function loadLegacyCatalogDetail(slug: string): Promise<LegacyCatalogDetail> {
    const [{ default: component }, { getCatalogEntry }] = await Promise.all([
      import('$lib/component-catalog/CatalogComponentPage.svelte'),
  ...
  {#await legacyDetail then detail}
    ...
    {:else}
      <section class="w-full min-w-0 space-y-4 p-6 lg:p-10">
        ...
        <a class="text-sm font-medium text-primary-ink underline" href="/sandbox">Back to catalog</a
        >
      </section>
    {/if}
  {/await}
