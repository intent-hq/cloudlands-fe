...
const WORKSPACE_SURFACE_FILE = path.resolve(COMPOSABLES_DIR, '..', 'WorkspaceSurface.svelte');
...
describe('lifecycle-safe selector access in workspace-page composables', () => {
  ...
  it('WorkspaceSurface.svelte: event handlers must not use lifecycle-sensitive selector access', () => {
    const source = fs.readFileSync(WORKSPACE_SURFACE_FILE, 'utf-8');
    const handlerBodies = extractEventHandlerBodies(source);
    ...
    expect(allViolations, 'Lifecycle violations in WorkspaceSurface.svelte').toEqual([]);
  });

  it('WorkspaceSurface.svelte: template must not use one-shot .select() for reactive props', () => {
    const source = fs.readFileSync(WORKSPACE_SURFACE_FILE, 'utf-8');
  ...
  it('WorkspaceSurface.svelte: panel layout scope follows workspace prop changes', () => {
    const source = fs.readFileSync(WORKSPACE_SURFACE_FILE, 'utf-8');

    expect(source).toContain('const panelLayoutId = $derived(workspaceId);');
    expect(source).not.toMatch(/const\s+panelLayoutId\s*=\s*workspaceId\s*;/);
  });
});
