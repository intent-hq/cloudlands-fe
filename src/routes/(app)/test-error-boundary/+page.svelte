  ...
  <div class="max-w-6xl mx-auto">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div class="space-y-4">
        ...
        <ErrorBoundary
          componentName="BasicTest"
          fallback="Something went wrong in the basic boundary"
        >
          <div class="space-y-3">
            <Button variant="destructive" onclick={triggerBasicError} class="w-full">
              Trigger Simple Error
            </Button>
            <p class="text-xs text-subtle">Throws a basic Error with a custom message</p>

            <Button variant="destructive" onclick={triggerBasicTypeError} class="w-full">
              Trigger TypeError
            </Button>
            <p class="text-xs text-subtle">Attempts to call a method on null (TypeError)</p>
      ...
      <div class="space-y-4">
        ...
        <EnhancedErrorBoundary componentName="EnhancedTest" autoRecover={true} recoveryDelay={2000}>
          <div class="space-y-3">
            <Button variant="destructive" onclick={triggerEnhancedError} class="w-full">
              Trigger Error (Auto-Recover)
            </Button>
            <p class="text-xs text-subtle">Throws an error that will auto-retry up to 3 times</p>

            <Button variant="destructive" onclick={triggerEnhancedTypeError} class="w-full">
              Trigger TypeError (Auto-Recover)
            </Button>
            <p class="text-xs text-subtle">Throws a TypeError with automatic recovery</p>
