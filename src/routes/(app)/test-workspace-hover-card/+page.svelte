<script lang="ts">
  ...
  function makeTasks(
    ...
    return Array.from({ length: total }, (_, i) => ({
      ...
      title: `Task ${i + 1}`,
      status:
        i < completed ? 'complete' : i < completed + inProgress ? 'in_progress' : 'not_started',
    }));
  }
  ...
  function seedTasks(wsId: string, tasks: WorkspaceTask[]) {
    ...
    appStore.dispatch(loadWorkspaceTasksSucceeded(wsId, tasks, { total, completed, inProgress }));
  }
      ...
      variations: [
        ...
        },
        {
          ...
          workspace: workspace('overflow-agents', 'Agent overflow review', {
            ...
            statusMessage:
              'Several agents are active so designers can inspect list density and overflow spacing.',
          }),
        },
      ],
    },
    {
      ...
      variations: [
        ...
        },
        {
          ...
          description:
            'Uses task-level segments for complete, in-progress, waiting, and review-required.',
        ...
        <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {#each section.variations as variation (variation.label)}
            <article class="flex flex-col gap-3 {variation.frameClass ?? ''}">
              ...
              <div
                class="flex min-h-[220px] items-start justify-center bg-muted/20 p-4 shadow-inner"
              >
                <WorkspaceHoverCard
    ...
    <section>
      <div class="mb-4">
        ...
        <p class="mt-1 text-sm text-muted-foreground">
          ...
          content. The lower trigger should prefer the right side and clamp vertically near the viewport
          bottom; the right-edge trigger should flip the card to the left.
        </p>
      </div>
