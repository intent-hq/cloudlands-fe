...
- App sagas are collected in `src/lib/store/sagas.ts`.
- `src/lib/store/saga-registration.ts` registers app sagas with `store.registerSagas(...)` before Store initialization and starts app sagas explicitly through `store.runSaga(name)`.
...
7. Register app sagas through the app saga map and rely on the existing saga-registration bootstrap.
