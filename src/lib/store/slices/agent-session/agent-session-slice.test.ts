...
describe("agent-session-slice reducer", () => {
  ...
  describe("upsertSession", () => {
    ...
    it("overwrites existing session", () => {
      ...
      state = agentSessionReducer(state, upsertSession(makeSession("a1", "ws-1", { name: "Updated" })));
  ...
  describe("setSessionStreaming", () => {
    ...
    it("is no-op if value unchanged", () => {
      const state = agentSessionReducer(initialState, upsertSession(makeSession("a1", "ws-1", { isStreaming: true })));
  ...
  describe("addMessage", () => {
    ...
    it("prunes messages beyond 500", () => {
      ...
      let state = agentSessionReducer(initialState, upsertSession(makeSession("a1", "ws-1", { messages: [] })));
  ...
  describe("updateMessage", () => {
    ...
    it("is no-op for unknown message", () => {
      ...
      const next = agentSessionReducer(state, updateMessage("a1", "unknown", { role: "assistant" }));
      expect(next).toBe(state);
    });
  });
  ...
  describe("replaceMessages", () => {
    it("replaces all messages with dedup/prune", () => {
      const msg = makeMessage("m1");
      let state = agentSessionReducer(initialState, upsertSession(makeSession("a1", "ws-1", { messages: [makeMessage("old")] })));
  ...
  describe("updateDigest", () => {
    ...
    it("clears digest with null", () => {
      let state = agentSessionReducer(initialState, upsertSession(makeSession("a1", "ws-1", { digest: "old" })));
  ...
  describe("renameSession", () => {
    ...
    it("is no-op if name unchanged", () => {
      const state = agentSessionReducer(initialState, upsertSession(makeSession("a1", "ws-1", { name: "Same" })));
