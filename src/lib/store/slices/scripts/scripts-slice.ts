 ...
 *
 * Workspace-scoped state for script definitions, runtime states, and output.
 */
...
import type { WorkspaceScript, ScriptRuntimeState, ScriptsState, ScriptsWorkspaceState, ScriptOutputLine } from "./scripts-types";
...
/** Set loading state */
export const setScriptsLoading = createAction<[wsId: string, loading: boolean]>("scripts/setLoading");

/** Set initialized state */
export const setScriptsInitialized = createAction<[wsId: string, initialized: boolean]>("scripts/setInitialized");

/** Bulk set scripts and runtime states (from list response) */
export const setScriptsData = createAction(
  "scripts/setScriptsData",
  (wsId: string, scripts: Record<string, WorkspaceScript>, runtimeStates: Record<string, ScriptRuntimeState>) =>
    ({ wsId, scripts, runtimeStates }),
);

/** Upsert a single script definition */
export const upsertScript = createAction<[wsId: string, script: WorkspaceScript]>("scripts/upsertScript");
...
export const updateRuntimeState = createAction(
  ...
  (wsId: string, scriptId: string, partial: Partial<ScriptRuntimeState>) =>
    ({ wsId, scriptId, partial }),
);

/** Append output lines for a script */
export const appendScriptOutput = createAction<[wsId: string, scriptId: string, lines: ScriptOutputLine[]]>(
  "scripts/appendOutput",
);

/** Set output buffer for a script (used during init to load buffered output) */
export const setScriptOutput = createAction<[wsId: string, scriptId: string, lines: ScriptOutputLine[]]>(
  "scripts/setOutput",
);
...
export const scriptsReducer = createReducer<ScriptsState>(initialState)
  ...
  .with(setScriptsData, (state, { payload: { wsId, scripts, runtimeStates } }) => {
    const ws = getWorkspaceState(state, wsId);
    return setWorkspaceState(state, wsId, { ...ws, scripts, runtimeStates });
  })
  .with(upsertScript, (state, { payload: [wsId, script] }) => {
    const ws = getWorkspaceState(state, wsId);
    const runtimeStates = ws.runtimeStates[script.id]
      ? ws.runtimeStates
      : { ...ws.runtimeStates, [script.id]: createDefaultRuntimeState() };
  ...
  .with(removeScript, (state, { payload: [wsId, scriptId] }) => {
    ...
    const { [scriptId]: _r, ...runtimeStates } = ws.runtimeStates;
    const { [scriptId]: _o, ...outputBuffers } = ws.outputBuffers;
    return setWorkspaceState(state, wsId, { ...ws, scripts, runtimeStates, outputBuffers });
  })
  .with(updateRuntimeState, (state, { payload: { wsId, scriptId, partial } }) => {
    const ws = getWorkspaceState(state, wsId);
    const current = ws.runtimeStates[scriptId] ?? createDefaultRuntimeState();
    return setWorkspaceState(state, wsId, {
      ...ws,
      runtimeStates: { ...ws.runtimeStates, [scriptId]: { ...current, ...partial } },
    });
  })
