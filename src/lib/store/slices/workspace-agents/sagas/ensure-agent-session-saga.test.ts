import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { expectSaga } from "redux-saga-test-plan";
import * as matchers from "redux-saga-test-plan/matchers";

vi.mock("typed-redux-saga", async () =>
  await import("$lib/store/utils/test-helpers/typed-redux-saga-mock"),
);

const { restoreSessionWithoutBackendMock } = vi.hoisted(() => ({
  restoreSessionWithoutBackendMock: vi.fn(async () => null),
}));

vi.mock("$features/agent/agent-ipc-bridge", () => ({
  agentService: {
    restoreSessionWithoutBackend: restoreSessionWithoutBackendMock,
  },
}));

import {
  selectCurrentWorkspace,
  selectWorkspaceById,
} from "$lib/store/slices/workspace/workspace-selectors";
import { selectAgentById } from "../workspace-agents-selectors";
import { ensureAgentSessionLoaded } from "../workspace-agents-slice";
import {
  _resetInFlightEnsureKeysForTest,
  handleEnsureAgentSessionLoaded,
  watchEnsureAgentSessionLoadedSaga,
} from "./ensure-agent-session-saga";

const WS_ID = "ws-1";
const AGENT_ID = "agent-1";

const mockWorkspace = {
  id: WS_ID,
  branch: "feature/test",
  repositoryOwner: "o",
  repositoryName: "r",
} as any;

const mockSession = {
  id: AGENT_ID,
  workspaceId: WS_ID,
  messages: [],
} as any;

describe("ensureAgentSessionLoaded saga", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    _resetInFlightEnsureKeysForTest();
  });

  afterEach(() => {
    _resetInFlightEnsureKeysForTest();
  });

  describe("handleEnsureAgentSessionLoaded", () => {
    it("no-ops when the session already exists (hit)", async () => {
      await expectSaga(handleEnsureAgentSessionLoaded, WS_ID, AGENT_ID)
        .provide([
          [matchers.select.selector(selectAgentById.select), mockSession],
        ])
        .silentRun(50);

      expect(restoreSessionWithoutBackendMock).not.toHaveBeenCalled();
    });

    it("calls restoreSessionWithoutBackend when the session is missing (miss)", async () => {
      await expectSaga(handleEnsureAgentSessionLoaded, WS_ID, AGENT_ID)
        .provide([
          [matchers.select.selector(selectAgentById.select), undefined],
          [matchers.select.selector(selectWorkspaceById.select), mockWorkspace],
          [matchers.call.fn(restoreSessionWithoutBackendMock), null],
        ])
        .call.fn(restoreSessionWithoutBackendMock)
        .silentRun(50);
    });

    it("falls back to selectCurrentWorkspace when selectWorkspaceById returns undefined", async () => {
      await expectSaga(handleEnsureAgentSessionLoaded, WS_ID, AGENT_ID)
        .provide([
          [matchers.select.selector(selectAgentById.select), undefined],
          [matchers.select.selector(selectWorkspaceById.select), undefined],
          [matchers.select.selector(selectCurrentWorkspace.select), mockWorkspace],
          [matchers.call.fn(restoreSessionWithoutBackendMock), null],
        ])
        .call.fn(restoreSessionWithoutBackendMock)
        .silentRun(50);
    });

    it("does nothing when no workspace can be resolved", async () => {
      await expectSaga(handleEnsureAgentSessionLoaded, WS_ID, AGENT_ID)
        .provide([
          [matchers.select.selector(selectAgentById.select), undefined],
          [matchers.select.selector(selectWorkspaceById.select), undefined],
          [matchers.select.selector(selectCurrentWorkspace.select), undefined],
        ])
        .silentRun(50);

      expect(restoreSessionWithoutBackendMock).not.toHaveBeenCalled();
    });

    it("swallows errors from restoreSessionWithoutBackend", async () => {
      await expectSaga(handleEnsureAgentSessionLoaded, WS_ID, AGENT_ID)
        .provide([
          [matchers.select.selector(selectAgentById.select), undefined],
          [matchers.select.selector(selectWorkspaceById.select), mockWorkspace],
          [
            matchers.call.fn(restoreSessionWithoutBackendMock),
            Promise.reject(new Error("boom")),
          ],
        ])
        .silentRun(50);
    });
  });

  describe("watchEnsureAgentSessionLoadedSaga", () => {
    it("debounces duplicate dispatches for the same (wsId, agentId) while a load is in flight", async () => {
      // A never-resolving restore keeps the first worker in-flight for the
      // entire test run, so the in-flight Set must suppress the 2nd/3rd
      // dispatches. If debouncing is broken, we would see 3 calls instead of 1.
      restoreSessionWithoutBackendMock.mockImplementation(
        () => new Promise(() => {}),
      );

      await expectSaga(watchEnsureAgentSessionLoadedSaga)
        .provide([
          [matchers.select.selector(selectAgentById.select), undefined],
          [matchers.select.selector(selectWorkspaceById.select), mockWorkspace],
        ])
        .dispatch(ensureAgentSessionLoaded(WS_ID, AGENT_ID))
        .dispatch(ensureAgentSessionLoaded(WS_ID, AGENT_ID))
        .dispatch(ensureAgentSessionLoaded(WS_ID, AGENT_ID))
        .silentRun(100);

      expect(restoreSessionWithoutBackendMock).toHaveBeenCalledTimes(1);
    });

    it("allows concurrent loads for different (wsId, agentId) pairs", async () => {
      restoreSessionWithoutBackendMock.mockImplementation(
        () => new Promise(() => {}),
      );

      await expectSaga(watchEnsureAgentSessionLoadedSaga)
        .provide([
          [matchers.select.selector(selectAgentById.select), undefined],
          [matchers.select.selector(selectWorkspaceById.select), mockWorkspace],
        ])
        .dispatch(ensureAgentSessionLoaded(WS_ID, AGENT_ID))
        .dispatch(ensureAgentSessionLoaded(WS_ID, "agent-2"))
        .silentRun(100);

      expect(restoreSessionWithoutBackendMock).toHaveBeenCalledTimes(2);
    });
  });
});

