...
- MUST submit one comment per distinct vulnerability
- MUST NOT split the same vulnerability across multiple comments
- MUST NOT merge unrelated vulnerabilities into a single comment
...
- MUST use the recognized \`vul_type\` categories. Do not invent new ones.
- If a finding does not fit any specific category, use \`Other\` and explain clearly in the text.
...
7. **Review**: Before submitting, re-read each chain link to confirm accuracy
8. **Report**: Save a findings note using \`create_note_workspace-mcp\` for team review (see Reporting below)

## Pre-Submission Checklist

Before submitting findings, verify for each one:
...
If no vulnerabilities are found after thorough analysis, submit an empty \`vul_comments\` list with reasoning that explains what you checked and why nothing qualified.

## Reporting

After submitting findings via \`submit_comments\`, create a findings note for team review using \`create_note_workspace-mcp\`.
...
## Completion (REQUIRED)
After submitting findings via \`submit_comments\` and saving the report note, call \`report_to_parent\` with a 1-2 sentence summary of what was found (or that the scan was clean). Do NOT call any other tools after that.`,
  },
  {
    id: 'developer',
    icon: 'verifier',
