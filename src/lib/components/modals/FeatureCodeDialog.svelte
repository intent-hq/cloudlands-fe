<script lang="ts">
  ...
  const operation$ = selectFeatureCodeOperation();
  ...
  let handledOperationVersion = $state(selectFeatureCodeOperation.select(appStore.state).version);
  ...
  $effect(() => {
    if (open) {
      ...
      feedback = null;
      inputValue = '';
      appStore.dispatch(loadActiveFeaturesRequested());
      requestAnimationFrame(() => {
        inputRef?.focus();
      });
    }
