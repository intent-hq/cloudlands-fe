      ...
      {:else}
        ...
        >
          ...
          {#if (!agentAttribution && !automatedWakePresentation) || isAgentMessageExpanded}
            <div
              class="absolute right-1 z-10 opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100 {agentAttribution
                ? 'bottom-1'
                : 'top-1'}"
            >
              <div
                ...
                {#each parsedMessage.segments as segment, i (i)}
                  ...
                  {:else if segment.type === 'mention'}
                    ...
                    >
                      ...
                      {/if}
                      <span class="max-w-[180px] truncate" title={segment.label}
                        >{segment.label}</span
                      >
                    </button>
                  {/if}
