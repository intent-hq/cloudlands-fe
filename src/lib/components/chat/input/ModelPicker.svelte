<script lang="ts">
  ...
  const providerCatalogs$ = selectProviderModelsCacheMap();
  const providerRequests$ = selectProviderModelsRequests();
  const catalogObserverId = crypto.randomUUID();
  const catalogProviderIds = $derived(
    $modelFetchProviderIds$.filter(canUseProviderModels).map(normalizeProviderId),
  );
  ...
  $effect(() => {
    appStore.dispatch(providerModelsObserved(catalogObserverId, observedProviderIds));
  });
  ...
  function handleRefreshProvider(providerId: string) {
    ...
    appStore.dispatch(providerModelsRequested(providerId, 'refresh'));
  }

  function handleRetry() {
    for (const providerId of observedProviderIds) {
      appStore.dispatch(providerModelsRequested(providerId, 'retry'));
    }
  }
  ...
  $effect(() => {
    ...
    appStore.dispatch(providerModelsRequested(currentProvider, 'silentRetry'));
  });
