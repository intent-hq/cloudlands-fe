...
<div class="space-y-4">
  ...
  <div class="space-y-3">
    {#each additionalProviders as provider (provider.id)}
      <div class="flex items-center justify-between py-2">
        <div>
          ...
          <p class="text-xs text-muted-foreground">
            Use <code class="px-1 py-0.5 bg-muted rounded text-[11px]">{provider.command}</code> CLI as
            an agent
          </p>
        </div>
        <Toggle
    ...
    {#if additionalProviders.length === 0}
      <p class="text-sm text-muted-foreground italic">No additional agents available</p>
    {/if}
  </div>
</div>
