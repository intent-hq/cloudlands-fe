...
warmImport(() => import('./HardwareConsoleDeviceSvg.svelte'));
...
describe('HardwareConsoleDeviceSvg', () => {
  ...
  it('opens the knob explainer dialog', async () => {
    ...
    expect(
      result.getByRole('dialog', { name: m.settings_hardware_knobExplainer_label() }),
    ).toBeTruthy();
  });

  it('opens the joystick explainer dialog', async () => {
    const { HardwareConsoleDeviceSvg, m } = await loadComponent();
    const result = render(HardwareConsoleDeviceSvg);
  ...
  it('keeps the Codex Micro linked mic pair independently selectable', async () => {
    const { HardwareConsoleDeviceSvg, m } = await loadComponent();
