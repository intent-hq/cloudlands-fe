...
for (const zoomFactor of [1, 2]) {
  test(`Mod+W uses live browser geometry at ${zoomFactor * 100}% zoom`, async ({ mount, page }) => {
    await page.emulateMedia({ reducedMotion: 'reduce' });
    const component = await mount(PanelModWShortcutHarness, { props: { zoomFactor } });
    ...
    await expect(layoutState).toHaveAttribute(
      'data-tab-order',
      `other-workspace,mod-w-browser-${zoomFactor}-3`,
    );
    await expect(layoutState).toHaveAttribute('data-current-tab', `mod-w-browser-${zoomFactor}-3`);
