<script lang="ts">
  ...
  let {
    open,
    title = m.workspaceCreation_dirPicker_selectFolder_label(),
  ...
  const selectButtonLabel = $derived(
    selectTargetName !== null
      ? m.workspaceCreation_dirPicker_selectNamed_label({ name: selectTargetName })
      : selectLabel,
  );
  ...
  >
    <header class="flex h-12 shrink-0 items-center justify-between border-b border-border px-4">
      ...
      <button
        ...
        onclick={onClose}
        aria-label={m.workspaceCreation_dirPicker_close_ariaLabel()}
      >
    ...
    <div class="flex min-h-0 flex-1">
      <aside class="hidden w-44 shrink-0 border-r border-border bg-muted/15 px-2 py-3 sm:block">
        <h3 class="mb-1 px-2 text-xs font-semibold text-muted-foreground">
          {m.workspaceCreation_dirPicker_favorites_label()}
        </h3>
        <nav class="space-y-0.5" aria-label={m.workspaceCreation_dirPicker_favorites_label()}>
      ...
      <div class="flex min-w-0 flex-1 flex-col">
        ...
        >
          <button
            ...
            onclick={navigateUp}
            aria-label={m.workspaceCreation_dirPicker_goUp_ariaLabel()}
            title={m.workspaceCreation_dirPicker_goUp_tooltip()}
          >
          ...
          {#if pathEditing}
            <input
              ...
              aria-label={m.workspaceCreation_dirPicker_path_ariaLabel()}
          ...
          {:else}
            <nav
              ...
              aria-label={m.workspaceCreation_dirPicker_breadcrumbs_ariaLabel()}
            >
            ...
            <button
              ...
              aria-label={m.workspaceCreation_dirPicker_editPath_ariaLabel()}
              title={m.workspaceCreation_dirPicker_editPath_ariaLabel()}
            >
          ...
          <label class="relative block w-28 shrink-0 sm:w-40">
            ...
            <input
              ...
              placeholder={m.workspaceCreation_dirPicker_search_placeholder()}
              aria-label={m.workspaceCreation_dirPicker_search_ariaLabel()}
              autocomplete="off"
            />
          </label>
        </div>
        ...
        <div
          ...
          role="listbox"
          aria-label={m.workspaceCreation_dirPicker_contents_ariaLabel()}
        >
          {#if loading}
            <div class="flex items-center justify-center gap-2 py-12 text-sm text-muted-foreground">
              ...
              <span>{m.workspaceCreation_dirPicker_loading_label()}</span>
            </div>
          {:else if error}
            <div class="px-5 py-10 text-sm text-danger">
              <p class="mb-1 font-medium">{m.workspaceCreation_dirPicker_readError_title()}</p>
          ...
          {:else if visibleEntries.length === 0}
            <div class="px-4 py-12 text-center text-sm text-muted-foreground">
              {searchDraft.trim()
                ? m.workspaceCreation_dirPicker_noSearchResults_description()
                : showFiles
                  ? m.workspaceCreation_dirPicker_emptyFolder_description()
                  : m.workspaceCreation_dirPicker_noSubfolders_description({
              ...
              {#each visibleEntries as entry, index (entry.path)}
                ...
                <li>
                  ...
                  >
                    ...
                    {#if entry.isGitRepo}
                      <span title={m.workspaceCreation_dirPicker_gitRepository_tooltip()}>
    ...
    <footer class="flex shrink-0 items-center gap-2 border-t border-border bg-muted/10 px-4 py-3">
      {#if mode === 'directory' && onCreateDirectory}
        {#if newFolderOpen}
          <div class="flex min-w-0 flex-1 items-center gap-1.5">
            <input
              ...
              placeholder={m.workspaceCreation_dirPicker_newFolderName_placeholder()}
              aria-label={m.workspaceCreation_dirPicker_newFolderName_ariaLabel()}
        ...
        {:else}
          ...
          >
            {m.workspaceCreation_dirPicker_newFolder_label()}
          </button>
        {/if}
      {/if}
      <button
        ...
        onclick={onClose}
      >
        {m.workspaceCreation_dirPicker_cancel_label()}
      </button>
      <button
        type="button"
        class={cn(
