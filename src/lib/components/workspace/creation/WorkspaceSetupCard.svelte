<script lang="ts">
   ...
   * Used both during workspaceCreation (replacing the form) and at the top of the chat panel.
   *
  ...
  function copyToClipboard(text: string, label: string) {
    ...
    toast.success(m.workspaceCreation_setupCard_copied_label({ label }));
  }
  ...
  <div class="space-y-0.5 pb-4">
    ...
    {#snippet repoNameCopyable()}
      {#if repoPath}
        {@render copyableRef(
          repoName,
          m.workspaceCreation_setupCard_originalFolder_label(),
          repoPath,
        )}
      {:else}
        {repoName}
    ...
    {#snippet repoActive()}
      ...
      {:else if skipIsolation}
        {m.workspaceCreation_setupCard_opening_before()}
        {@render repoNameCopyable()}{m.workspaceCreation_setupCard_opening_after()}
      {:else}
        {m.workspaceCreation_setupCard_creatingIsolatedCopy_before()} {@render repoNameCopyable()}
      {/if}
    {/snippet}
    {#snippet repoDone()}
      {#if skipIsolation}
        {m.workspaceCreation_setupCard_workingDirectlyOn_before()}
        <code class="text-sm bg-secondary py-1 px-1.5">{displayBranch}</code>
        {#if worktreePath}{' '}{m.workspaceCreation_setupCard_at_label()}
          <OpenComboButton
      ...
      {:else}
        {m.workspaceCreation_setupCard_createdIsolatedCopy_before()}
    ...
    {#snippet branchActive()}
      {#if skipIsolation}
        {#if branch}
          {m.workspaceCreation_setupCard_workingOnBranchNamed_before()}
          <span class="">{branch}</span
          >{m.workspaceCreation_setupCard_workingOnBranchActive_after()}
        {:else}
          {m.workspaceCreation_setupCard_workingOnBranch_label()}
        {/if}
      {:else if branch}
        {m.workspaceCreation_setupCard_creatingBranch_before()} <span class="">{branch}</span>
        {m.workspaceCreation_setupCard_creatingBranch_middle()}
        <button
          class="underline underline-offset-2 cursor-pointer hover:text-foreground transition-colors"
          onclick={() => copyToClipboard(baseRef, m.workspaceCreation_setupCard_baseRef_label())}
          >{baseRef}</button
        >{m.workspaceCreation_setupCard_creatingBranch_after()}
      {:else}
        {m.workspaceCreation_setupCard_creatingBranchNoName_label()}
      {/if}
    {/snippet}
    ...
    {#snippet branchDone()}
      {#if skipIsolation}
        {#if branch}
          ...
          {@render copyableRef(
            branch,
            m.workspaceCreation_setupCard_branchName_label(),
          )}{m.workspaceCreation_setupCard_workingOnBranchDone_after()}
        {:else}
          {m.workspaceCreation_setupCard_workingDirectlyOnBranch_label()}
        {/if}
      {:else if branch}
        {m.workspaceCreation_setupCard_workingInNewBranch_before()}
        {@render copyableRef(
          branch,
          m.workspaceCreation_setupCard_branchName_label(),
        )}{m.workspaceCreation_setupCard_workingInNewBranch_middle()}
        {@render copyableRef(
          baseRef,
          m.workspaceCreation_setupCard_baseRef_label(),
        )}{m.workspaceCreation_setupCard_workingInNewBranch_after()}
      {:else}
        {m.workspaceCreation_setupCard_branchCreated_label()}
      {/if}
    {/snippet}
    ...
    {#snippet setupActive()}
      ...
      {#if projectType}<span class="">{projectType}</span
        >{:else}{m.workspaceCreation_setupCard_project_label()}{/if}
      {m.workspaceCreation_setupCard_runningSetup_after()}
    {/snippet}
    {#snippet setupDone()}
      {m.workspaceCreation_setupCard_ranSetup_before()}
      {#if projectType}<span class="">{projectType}</span
        >{:else}{m.workspaceCreation_setupCard_project_label()}{/if}
      {m.workspaceCreation_setupCard_ranSetup_middle()}{#if onFocusSetupTerminal}{' '}{m.workspaceCreation_setupCard_ranSetupIn_middle()}
        <TooltipRich side="bottom" align="start" interactive maxWidth="22rem" delayDuration={300}>
          {#snippet trigger()}
            <button
              class="underline underline-offset-2 cursor-pointer hover:text-foreground transition-colors"
              onclick={onFocusSetupTerminal}
              >{m.workspaceCreation_setupCard_terminalTab_label()}</button
            >
          {/snippet}
          {#snippet content()}
          ...
          {#snippet footer()}
            <span class="text-xs text-muted-foreground opacity-50"
              >{m.workspaceCreation_setupCard_openTerminal_footer()}</span
            >
          {/snippet}
        </TooltipRich>{/if}{m.workspaceCreation_setupCard_ranSetup_after()}
    {/snippet}
    ...
    {#snippet specialistWithTooltip()}
      <TooltipRich side="bottom" align="start" interactive maxWidth="22rem" delayDuration={300}>
        ...
        {#snippet footer()}
          <span class="text-xs text-muted-foreground opacity-50">
            {m.workspaceCreation_setupCard_editInSettings_footer()}
          </span>
        {/snippet}
      </TooltipRich>
    {/snippet}
    {#snippet agentActive()}
      {#if !hasPrompt && specialistId}
        {m.workspaceCreation_setupCard_agentReadyNamed_before()}
        {@render specialistWithTooltip()}
        {m.workspaceCreation_setupCard_agentReadyNamed_after()}
      {:else if !hasPrompt}
        {m.workspaceCreation_setupCard_agentReady_label()}
      {:else if specialistId === 'spec-writer'}
        {m.workspaceCreation_setupCard_specStartingUp_before()}
        {@render specialistWithTooltip()}
        {m.workspaceCreation_setupCard_specStartingUp_after()}
      {:else if specialistId}
        {m.workspaceCreation_setupCard_startingUpNamed_before()}
        {@render specialistWithTooltip()}
        {m.workspaceCreation_setupCard_startingUpNamed_after()}
      {:else}
        {m.workspaceCreation_setupCard_startingUp_label()}
      {/if}
    {/snippet}
    {#snippet agentDone()}
      {#if !hasPrompt && specialistId}
      ...
      {:else if !hasPrompt}
        {m.workspaceCreation_setupCard_agentReady_label()}
      {:else if specialistId === 'spec-writer'}
        {m.workspaceCreation_setupCard_specDone_before()}
        {@render specialistWithTooltip()}
        {m.workspaceCreation_setupCard_specDone_after()}
      {:else if specialistId}
        {m.workspaceCreation_setupCard_agentDoneNamed_before()}
        {@render specialistWithTooltip()}
        {m.workspaceCreation_setupCard_agentDoneNamed_after()}
      {:else}
        {m.workspaceCreation_setupCard_agentOrienting_label()}
      {/if}
    {/snippet}
  </div>
</div>
