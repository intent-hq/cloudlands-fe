...
vi.mock('$store/renderer/slices/workspace-notes/workspace-notes-slice', () => ({
  updateNoteContent: vi.fn(
    (workspaceId: string, noteId: string, content: string, immediate?: boolean) => ({
      ...
      payload: [workspaceId, noteId, content, immediate],
    }),
  ),
  coordinateExternalNoteUpdate: vi.fn(
    (workspaceId: string, noteId: string, updateVersion: number) => ({
      type: 'workspaceNotes/coordinateExternalNoteUpdateMutation',
      payload: [workspaceId, noteId, updateVersion],
  ...
  cancelExternalNoteUpdateCoordination: vi.fn((workspaceId: string, noteId: string) => ({
    type: 'workspaceNotes/cancelExternalNoteUpdateCoordination',
...
describe('NoteWithComments task conversion regression', () => {
  ...
  it('dispatches rich-editor autosaves through the canonical typed action and flushes on teardown', async () => {
    replaceNotes([createNote('autosave-note', 'Autosave', 'Baseline content')]);
    const view = await renderInitializedNote('autosave-note', 'Baseline content');
    await waitFor(() => expect(editorInstances.at(-1)).toBeDefined());
    ...
    expect(mockDispatch).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        payload: [
          ...
          { immediate: false, baseRev: undefined, baseContent: 'Baseline content' },
        ],
      }),
    );
    ...
    expect(mockDispatch).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        payload: [
          ...
          'autosave-note',
          expect.stringContaining('edited'),
          { immediate: true, baseRev: undefined, baseContent: expect.stringContaining('edited') },
        ],
      }),
    );
  });
