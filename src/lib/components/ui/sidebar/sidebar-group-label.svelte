<script lang="ts">
  ...
  const mergedProps = $derived({
    class: cn(
      'group/group-label text-muted-foreground/70 hover:text-muted-foreground outline-none flex h-8 w-full shrink-0 items-center justify-start gap-2 rounded-md px-2 text-xs font-normal transition-colors duration-spring-fast ease-spring-fast motion-reduce:transition-none focus-visible:ring-1 focus-visible:ring-focus-ring [&>svg]:size-4 [&>svg]:shrink-0',
