// @verify-changed-triggers: ./CodeEditor.svelte
...
describe('CodeEditor external content sync source guard', () => {
  const source = readFileSync(
    join(process.cwd(), 'src/lib/components/editor/CodeEditor.svelte'),
    'utf-8',
  );
  ...
  it('gates the file-too-large Open-in-VS-Code and Reveal buttons on workspace host locality (monorepo#2171)', () => {
    ...
    expect(source).toContain('{#if resolvedWorkspaceId && filePath && $isWorkspaceHostLocal$}');
    // No ungated variant of the block remains.
    expect(source).not.toContain('{#if workspaceId && filePath}');
  });

  it('does not read a workspace from the legacy active-workspace selector', () => {
    expect(source).not.toContain('selectActiveWorkspace');
    expect(source).toContain('getWorkspaceRouteContext');
    expect(source).toContain('selectWorkspaceById(workspaceIdStore)');
  });
});
