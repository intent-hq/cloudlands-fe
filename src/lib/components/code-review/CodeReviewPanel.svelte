<script lang="ts">
  ...
  const routeWorkspaceId = getWorkspaceRouteContext()?.workspaceId ?? null;
  const workspaceId$ = toStore(() => workspaceId ?? routeWorkspaceId ?? '');
  const workspace$ = selectWorkspaceById(workspaceId$);

  function getWorkspaceSnapshot() {
    const id = $workspaceId$;
    return id ? selectWorkspaceById.select(appStore.state, id) : undefined;
  }
  ...
  let hasLoadedFromStore = $state(false);

  // Walkthrough executor - reactive state from Redux
  // We need the workspace ID to create a selector subscription
  const walkthroughWorkspaceId = $derived($workspaceId$);

  // Reset workspace-scoped walkthrough state when this mounted panel is reused.
  let previousWalkthroughWorkspaceId = $state<string | null | undefined>(undefined);
  $effect(() => {
    const currentWorkspaceId = walkthroughWorkspaceId || null;
    if (previousWalkthroughWorkspaceId === currentWorkspaceId) return;

    previousWalkthroughWorkspaceId = currentWorkspaceId;
    walkthrough = null;
    walkthroughStatus = 'idle';
    walkthroughError = '';
    hasAutoGeneratedWalkthrough = false;
  ...
  // Load persisted walkthrough on mount (only once, reads from bgExecutor which is now persisted)
  $effect(() => {
    ...
    {#if hasError && error}
      <div class="p-4 border-b border-border" transition:fly={{ y: 4, duration: 200 }}>
        ...
        >
          ...
          {#if onRerun}
            <Button variant="ghost" size="xs" class="mt-2 text-danger" onclick={onRerun}>
              <Fa icon={faRotateRight} class="h-3 w-3 mr-1" />
