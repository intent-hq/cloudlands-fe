<script lang="ts">
  ...
  const workspacePath = $derived(
    $activeWorkspace?.worktreePath || $activeWorkspace?.repositoryPath,
  );
  ...
  const walkthroughWorkspaceId = $derived($activeWorkspace?.id ?? '');
  ...
  $effect(() => {
    ...
    const workspace = getActiveWorkspaceSnapshot();
    if (!workspace) return;
