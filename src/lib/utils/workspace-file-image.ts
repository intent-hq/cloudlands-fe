 ...
 * checkout (image extensions only; SVG is deliberately excluded).
 *
...
/**
 * Rewrite `<img src="intent://.../file/...">` sources in rendered markdown
 * HTML to `workspace-file://` URLs. Non-file intent sources and invalid file
 * links are left untouched (the image simply falls back to its alt text).
 */
export function rewriteIntentFileImageSrcs(html: string, currentWorkspaceId?: string): string {
  ...
  return html.replace(
    ...
    (match, prefix: string, rawSrc: string, suffix: string) => {
      ...
      const rewritten = intentFileImageUrlToWorkspaceFileUrl(src, currentWorkspaceId);
      return rewritten ? `${prefix}${escapeAttr(rewritten)}${suffix}` : match;
    },
  );
}
