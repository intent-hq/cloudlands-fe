...
describe('LiveNotesClient mutations (fake transport)', () => {
  ...
  it('maps a -32005 conflict to a structured outcome with the normalized current note', async () => {
    rejectWith({
      ...
      data: {
        code: 'conflict',
        current: { id: 'note-1', title: 'Server', content: 'srv', rev: 7 },
      },
    });
    const client = new LiveNotesClient();
  ...
  it('listVersions calls note.listVersions then batches note.getVersion per entry (FE-shape)', async () => {
    ...
    mockedRequest.mockImplementation(async (method: string, params: Record<string, unknown>) => {
      ...
      if (method === 'note.getVersion') {
        ...
        return {
          type: 'snapshot',
          v,
          date: `2026-01-0${v}T00:00:00.000Z`,
          author: { id: 'x', name: 'X', type: v === 1 ? 'user' : 'agent' },
          title: `T${v}`,
          content: v === 1 ? 'body' : 'body-2',
        };
      }
    ...
    expect(mockedRequest).toHaveBeenCalledWith('note.listVersions', {
      workspaceId: 'ws-1',
      noteId: 'note-1',
    });
    expect(mockedRequest).toHaveBeenCalledWith('note.getVersion', {
      workspaceId: 'ws-1',
      noteId: 'note-1',
      v: 1,
    });
...
// ---- Typed per-workspace note channel (PROTOCOL §6.9, monorepo#1697) -------
// `subscribe` registers ONE `note.subscribe` per workspace id
// (`{ workspaceId }`), sourced from the same `workspace.list` enumeration —
// the sole data path; there is no legacy `note:*` events-driven refetch.
...
describe('LiveNotesClient.subscribe typed per-workspace note channel (PROTOCOL §6.9)', () => {
  ...
  it('a created workspace registers a new channel and merges its snapshot', async () => {
    ...
    const unsubscribe = client.subscribe(handler);
    await vi.waitFor(() =>
      expect(requestsFor('note.subscribe')).toEqual([{ workspaceId: 'ws-1' }]),
    );
    await flush();
    pushSnapshot('chan-1', 0, [wireNote('a', 'ws-1', 'A')]);
  ...
  it('retries a failed channel registration with backoff until it succeeds', async () => {
    ...
    try {
      ...
      expect(requestsFor('note.subscribe')).toHaveLength(3);
      unsubscribe();
    } finally {
      vi.useRealTimers();
    }
