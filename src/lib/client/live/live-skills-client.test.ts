...
describe('LiveSkillsClient', () => {
  ...
  describe('list', () => {
    ...
    it('rejects on transport error so callers can expose the failure', async () => {
      vi.spyOn(backendTransport, 'backendRequest').mockRejectedValue(
  ...
  describe('subscribe', () => {
    ...
    it('refetches and emits fresh skill list on skills:changed event', async () => {
      ...
      const backendRequestSpy = vi
        ...
        .mockResolvedValue(refreshedSkills);
    ...
    it('handles subscription registration failure gracefully', async () => {
      vi.spyOn(backendTransport, 'backendSubscribe').mockRejectedValue(
        ...
        const skills = [
          {
            name: 'a-skill',
            description: 'd',
            location: '/w/.intent/skills/a/SKILL.md',
