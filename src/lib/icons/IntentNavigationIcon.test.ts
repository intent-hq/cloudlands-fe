...
} satisfies Partial<Record<IntentNavigationIconName, string[]>>;

describe('IntentNavigationIcon', () => {
  it('exposes the approved icon names', () => {
    expect(intentNavigationIconNames).toEqual([
      'spaces',
      'tabs',
      'settings',
      'dandelion',
    ...
    expectTypeOf<IntentNavigationIconName>().toEqualTypeOf<
      'spaces' | 'tabs' | 'settings' | 'dandelion' | 'sidebar'
    >();
  });

  it.each(Object.keys(expectedGeometry) as (keyof typeof expectedGeometry)[])(
    'renders the exact %s vector with currentColor',
    (name) => {
