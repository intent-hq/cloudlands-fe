...
`src/features/agent/agent-stream-lifecycle.ts` now acts as a stream adapter. It registers/cleans stream handlers, accumulates raw stream payload pieces, forwards legacy window events where needed, and dispatches typed Redux actions such as `agentStreamUpdateReceived(...)` and `agentStreamResetStreamingMessagesRequested(...)`.
...
2. `agent-stream-lifecycle.ts` receives stream callbacks and dispatches raw Redux stream actions with IDs, content, content blocks, `appMessageId`, and stream metadata.
