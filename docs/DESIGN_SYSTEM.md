...
For a picker portalled into its owning dialog, use `ContentDialog`'s `allowOverflow`
option so the child can extend outside the shell; the body still scrolls independently.
