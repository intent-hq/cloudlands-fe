...
The following moves have been completed as part of the initial refactor wave:

| Module | Old Location | New Location | Status |
|--------|-------------|--------------|--------|
| `comment-id-generator.ts` | `src/features/comments/` | `src/shared/utils/` | ✅ Moved, shim in place |
| `notes-primitives-serializer.ts` | `src/features/notes/` | `src/lib/utils/` | ✅ Moved, shim in place |
| `workspace-validation.ts` | `src/features/workspace/main/` | `src/main/utils/` | ✅ Moved, shim in place |

Temporary re-export shims remain at the old paths. These will be removed once all consumers are confirmed migrated.
...
| `link-handler.ts` | Split | Extract `isAuthUrl()`/`isGitHubUrl()` → `src/lib/utils/link-url-utils.ts` |
| `workspace.client.ts` | Split | Extract `normalizeWorkspacePaths()` → `src/features/workspace/utils/` |
...
- [Component Responsibilities](./COMPONENT_RESPONSIBILITIES.md) — UI component organization