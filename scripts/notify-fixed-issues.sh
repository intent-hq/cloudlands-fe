...
#
# Usage: notify-fixed-issues.sh [--dry-run] <component> <version> <channel> \
#          <from-ref> <to-ref> [bundled-intentd-version]
#
...
#
# Env:
#   SOURCE_REPO      repo the range's PRs live on
...
usage="usage: notify-fixed-issues.sh [--dry-run] <component> <version> <channel> <from-ref> <to-ref> [bundled-intentd-version]"
COMPONENT="${1:?$usage}"
VERSION="${2:?$usage}"
CHANNEL="${3:?$usage}"
FROM_REF="${4:?$usage}"
...
if [[ -n "$BUNDLED_INTENTD" ]]; then
  ...
  if [[ ! "$BUNDLED_INTENTD" =~ ^[0-9]+\.[0-9]+\.[0-9]+(-[0-9A-Za-z.-]+)?$ ]]; then
