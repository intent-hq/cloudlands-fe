...
function buildTree(dirPath, name, rootPath, maxDepth = 50, currentDepth = 0) {
  if (currentDepth > maxDepth) return null;

  const stats = fs.statSync(dirPath);
  // Use relative path from root
  const relativePath = path.relative(path.dirname(rootPath), dirPath);
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        const child = buildTree(fullPath, entry, rootPath, maxDepth, currentDepth + 1);
        if (child) {
          children.push(child);
  ...
  return { name, path: relativePath, size: totalSize, children };
}
...
const tree = buildTree(resolvedPath, folderName, resolvedPath);
