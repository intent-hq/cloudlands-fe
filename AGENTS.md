...
| `agent.list` requests (unbounded on a large workspace)                  | `pnpm run lint:agent-list-scope` — `scripts/check-agent-list-scope.mjs` fails any non-test `agent.list` request (wire literal, `agents.list` / `listWithMeta`, saga `call` / tuple) without `scope` or `retiredOnly`; the file-level `ALLOWLIST` in the script needs a one-line reason and stale entries fail; CI runs it via `lint:architecture` (intent-hq/intent#5531)                                                                                                          |
