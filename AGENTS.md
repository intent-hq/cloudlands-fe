...
- **Pseudo-locale (`en-XA`)**: generated from `en.json` by `scripts/generate-pseudo-locale.mjs` (gitignored; regenerated by `generate:i18n`, `vite.config.mjs`, and `vitest.config.ts` — never hand-edit or translate it). Accented/expanded pseudo-English wrapped in `⟦…⟧`: unextracted strings show up as plain English, and the ~40% expansion surfaces truncation/layout issues. Selectable in the Settings language picker in dev builds only; system-locale matching never resolves to it.
